...
ACMD(do_drink)
{
   ...
   if(!strcmp(arg,"water") &&
	 IS_SET( world[ch->in_room].room_flags,DRINK_WATER|DRINK_POISON)){
...
ACMD(do_eat)
{
   ...
   if (!(food = get_obj_in_list_vis(ch, arg, ch->carrying,9999))) 
     if (!(food = 
	   get_obj_in_list_vis(ch, arg, world[ch->in_room].contents,9999))) 
       {
...
ACMD(do_pour)
{
   ...
   if (subcmd == SCMD_POUR) {
     ...
     if(IS_SET(world[ch->in_room].room_flags, DRINK_WATER|DRINK_POISON) &&
	!strcmp(arg1,"water")){
       if(IS_SET(world[ch->in_room].room_flags, DRINK_WATER))
	 ...
	 from_obj = &generic_poison;
       ...
       gener = 0;
       if(!(from_obj = 
	    get_obj_in_list_vis(ch, arg1,world[ch->in_room].contents,9999))){
	 if (!(from_obj = 
	       get_obj_in_list_vis(ch, arg1, ch->carrying,9999))) {
	   act("You can't find it!", FALSE, ch, 0, 0, TO_CHAR);
	   return;
	 }
       }
     ...
     if (from_obj->obj_flags.type_flag != ITEM_DRINKCON &&
	  from_obj->obj_flags.type_flag != ITEM_FOUNTAIN) {
   ...
   if (dotmode == FIND_ALL) {
	 ...
	 send_to_char("You're not using anything.\n\r", ch);
      ...
      if (!*arg)
	     ...
	     isname(arg, ch->equipment[i]->name, 0)) {
	       perform_remove(ch, i);
	       found = 1;
	    }
	 if (!found) {
      ...
      for (i = 0; i < MAX_WEAR; i++)
         if (ch->equipment[i]) 
	   if(isname(arg, ch->equipment[i]->name, 0)) break;
      if(i < MAX_WEAR)
	perform_remove(ch, i);
      else{
...
ACMD(do_light){
  ...
  if(!*arg) torch = ch->equipment[WEAR_LIGHT];
  else{
  ...
  if((torch->obj_flags.type_flag != ITEM_LIGHT) ||
     (torch->obj_flags.value[2] == 0)){
