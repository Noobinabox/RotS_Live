   ...
   for(nr = 0; nr < social_list_top; nr++)
     for(tmp = 0; tmp < social_list_top; tmp++){
       ...
       if(strncmp(soc_mess_list[tmp].command,
		  soc_mess_list[tmp+1].command,len1) > 0){
...
int find_action_old(char * arg){
  ...
  for(space_pos = 0; space_pos < tmp; space_pos++)
    if(arg[space_pos] <= ' ') break;
  space_char = arg[space_pos];
  arg[space_pos] = 0;

  for(tmp = 0; tmp <= social_list_top; tmp++)
    if(!strncmp(soc_mess_list[tmp].command,arg,strlen(arg))) break;

  arg[space_pos] = space_char;

  if(tmp > social_list_top) return -1;
  return tmp;
...
int
social_parser(char_data * ch, char * argument, waiting_type * wtl)
...
ACMD(do_action)
{
  ...
  if(wtl && (wtl->cmd == CMD_SOCIAL)) {  
    ...
    if((tmpwtl.targ1.type == TARGET_CHAR) && 
       char_exists(tmpwtl.targ1.ch_num))
