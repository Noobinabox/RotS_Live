...
ACMD(do_echo)
{
  ...
  char_data * tmpch;
   ...
   if (!*(argument + i))
	...
	if(tmpch != ch) send_to_char(buf, tmpch);
...
void do_stat_object(struct char_data *ch, struct obj_data *j)
{
  ...
  sprintf(buf,"Material: %s\n\r",
	  ((j->obj_flags.material >= 0) && 
	   (j->obj_flags.material < num_of_object_materials))?
	  object_materials[j->obj_flags.material]: "Unknown");
  send_to_char(buf,ch);
...
void do_stat_character(struct char_data *ch, struct char_data *k)
{
   ...
   send_to_char(buf, ch);
	...
	if(k->player.long_descr) send_to_char(k->player.long_descr,ch);
   ...
   sprintf(buf, "HP :[%d/%d+%d]  Stamina :[%d/%d+%d]  Move :[%d/%d+%d] Spirit:[%d/%d+%d]   Consts(hit/stamina/move): %d/%d/%d\n\r",
   ...
   if (IS_NPC(k)) {
      ...
      sprintf(buf, "NPC flags: (%ld) %s, agg flag: %ld, rp_flag: %d\n\r", MOB_FLAGS(k),
        buf2,(k)->specials2.pref, (k)->specials2.rp_flag);
      send_to_char(buf, ch);
   } else {
   ...
   if (IS_MOB(k)) {
      sprintf(buf, "Mob Spec-Proc: %s,  Asima: %d; Script: %d; ",
	      ...
	      ((!MOB_FLAGGED(k, MOB_SPEC)&&(k->specials.union1.prog_number))?
	       mobile_program_zone[PROG_NUMBER(k)]:-1), k->specials.script_number);
...
ACMD(do_zone)
{
  ...
  if(zonnum==0) zonnum=(int)(world[ch->in_room].number)/100;

  for(tmp=0; (tmp<=top_of_zone_table) && 
	(zone_table[tmp].number!=zonnum); tmp++);
  ...
  sprintf(buf,"Owners: ");
  tmpowner=zone_table[tmp].owners;
  if(tmpowner->owner==0) strcat(buf,"All");
  else
    while(tmpowner->owner!=0){
...
ACMD(do_purge)
{
   ...
   if(!strcmp(buf,"zone")){
	 ...
	 if((obj->in_room != NOWHERE) &&
	    (world[obj->in_room].zone == my_zone))	   
	   extract_obj(obj);
      }
...
ACMD(do_last)
{
   ...
   if(chdata.race >= MAX_RACES) chdata.race = 17;
...
ACMD(do_wizutil){
	...
	if(subcmd == 0)
		...
		for(tmp = 0; tmp < num_of_wizutils; tmp++)
			if(!str_cmp(wizutil_options[tmp].field,wtl->targ1.ptr.text->text)) break;
	...
	switch (subcmd)
		...
		case SCMD_REROLL:
			...
			if(vict->desc) act("$n just rerolled you.", FALSE, ch, 0, vict, TO_VICT);
		...
		case SCMD_NOTE:
			...
			while (*c!=' ') c++; // skip 'note'
			while (*c==' ') c++; // skip intermediate spaces
			while (*c!=' ') c++; // skip player name
			while (*c==' ') c++; // skip trailing spaces
			if ((!*c)||(!isalpha(*c)))
				{
...
ACMD(do_wizset)
{
  ...
  strcpy(buf, "Okay.");
   ...
   case 36:
     ...
     if (IS_RIDING(vict)) stop_riding(vict);
     char_from_room(vict);
     char_to_room(vict, i);
     break;
   case 37:
   ...
   case 51:
     ...
     break;
   case 52:
     GET_PROF_POINTS(PROF_MAGIC_USER,vict) = value;break;
   case 53:
     GET_PROF_POINTS(PROF_CLERIC,vict) = value;break;
   case 54:
     GET_PROF_POINTS(PROF_RANGER,vict) = value;break;
   case 55:
     GET_PROF_POINTS(PROF_WARRIOR,vict) = value;break;
   case 56:
...
ACMD(do_register){
  ...
  if(isdigit(*arg2)) zonnum = atoi(arg2);
  if(!*arg2) zonnum = world[ch->in_room].number/100;
  if(!*arg1 || !zonnum){
  ...
  if(!strncmp("mobile", arg1, strlen(arg1))){
    ...
    if(!sw) send_to_char("\n\r",ch);
    else if(!count) send_to_char("No mobiles there.\n\r",ch);
    return;
  }
  if(!strncmp("object", arg1, strlen(arg1))){
    ...
    if(!sw) send_to_char("\n\r",ch);
    else    if(!count) send_to_char("No objects there.\n\r",ch);
    return;
  }
  if(!strncmp("room", arg1, strlen(arg1))){
    ...
    if(!sw) send_to_char("\n\r",ch);
    else    if(!count) send_to_char("No rooms there.\n\r",ch);
    return;
  }
  ...
  if (!strncmp("top", arg1, strlen(arg1))){
        oldest = 0;
        if(zonnum > 25) zonnum = 25;
        if(zonnum < 1) zonnum = 1;
        race = 0;
        if(!*arg3) {
...
ACMD(do_findzone){
  ...
  if(isdigit(*arg)){
    ...
    for (i = 0; i <= top_of_zone_table; i++)
    if(((x < 0) || (x == zone_table[i].x)) &&
       ((y < 0) || (y == zone_table[i].y))){
...
ACMD(do_rehash){
  ...
  for(num = 0; num <= top_of_world; num++){
   ...
   for(tmpaf = world[num].affected; tmpaf; tmpaf = tmpaf->next){
     ...
     if((tmpaf->type == ROOMAFF_SPELL) &&
	(tmpaf->location >= 0) && (tmpaf->location != SPELL_NONE))
       perms_only = 0;
   }
 ...
 */
int
advance_perm(struct char_data *ch, struct char_data *vict, int level)
{
  if(GET_LEVEL(ch) == LEVEL_IMPL)
    return 1;
