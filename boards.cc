...
void	init_boards(void)
{
   ...
   board_info[0]= new  board_info_type
     (1112, 0, 0, LEVEL_GOD+1, MAX_BOARD_MESSAGES, "boa12",
      "Mobile board");
   board_info[1]=new  board_info_type
     (1111, 0, 0, LEVEL_GOD+1, MAX_BOARD_MESSAGES, "boa11",
      "Object board");
   board_info[2]=new  board_info_type
     (1110, 0, 0, LEVEL_GRGOD+1, MAX_BOARD_MESSAGES, "objects",
      "Zone assignment Board");
   board_info[3]=new  board_info_type
     (1109, 0, 0, LEVEL_GRGOD, MAX_BOARD_MESSAGES, "zones",
      "Whitie mortal Board");
   board_info[4]=new  board_info_type
     (1108, 0, 0, LEVEL_GOD+1, MAX_BOARD_MESSAGES, "mob",
      "Zone planning Board");
   board_info[5]=new  board_info_type
     (1107, 0, 0, LEVEL_GOD+1, MAX_BOARD_MESSAGES, "shaping",
      "Creepy Bug Board");
   board_info[6]=new  board_info_type
     (1106, 0, 0, LEVEL_GOD+1, MAX_BIG_BOARD_MESSAGES, "immort",
      "Immortal Board");
   board_info[7]=new  board_info_type
     (1105, 0, 0, LEVEL_GRGOD, MAX_BOARD_MESSAGES, "dim",
      "Dim's Board");
   board_info[8]=new  board_info_type
     (1104, 0, LEVEL_GOD+1, LEVEL_GRGOD+1, MAX_BIG_BOARD_MESSAGES, "general", 
      "News Board");
   board_info[9]= new  board_info_type
     (1119, 0, 0, LEVEL_GOD+1, MAX_BOARD_MESSAGES, "boa13", 
      "Uruk Board");
   board_info[10]= new  board_info_type
     (1118, LEVEL_GRGOD, LEVEL_GRGOD, LEVEL_GRGOD+1, MAX_BOARD_MESSAGES, "boa14", 
      "Flame Board");
   board_info[11]= new  board_info_type
     (1117, 0, 0, LEVEL_GOD+1, MAX_BOARD_MESSAGES, "boa15",
      "New World board");
   board_info[12]= new  board_info_type
     (1116, 0, 0, LEVEL_GOD+1, MAX_BOARD_MESSAGES, "boa16",
      "East board");
   board_info[13]= new  board_info_type
     (1114, 0, 0, LEVEL_GOD+1, MAX_BOARD_MESSAGES, "boa17",
      "Bright ideas Board");
   board_info[14]= new  board_info_type
     (1113, 0, 0, LEVEL_GRGOD, MAX_BOARD_MESSAGES, "boa18",
      "Prami Help Board");
   board_info[15]=new  board_info_type
     (1127, 0, 0, LEVEL_GOD+1, MAX_BOARD_MESSAGES, "lightic",
      "Light Races IC Board");
   board_info[16]=new  board_info_type
     (1128, 0, 0, LEVEL_GOD+1, MAX_BOARD_MESSAGES, "darkic",
      "Evil Races IC Board");
   board_info[17]=new  board_info_type
     (1129, 0, 0, LEVEL_GRGOD, MAX_BOARD_MESSAGES, "seether",
      "Seether's Board");
   board_info[18]=new  board_info_type
     (1130, 0, 0, LEVEL_GOD+1, MAX_BOARD_MESSAGES, "orcboard",
      "Orcish bulletin board");
   board_info[19]=new  board_info_type
     (1131, LEVEL_AREAGOD, LEVEL_AREAGOD, LEVEL_GRGOD+1, MAX_BOARD_MESSAGES, "cheaters",
      "Cheater board");
   board_info[20]=new  board_info_type
     (1132, 0, 0, LEVEL_GOD+1, MAX_BOARD_MESSAGES, "magiboard",
      "Magus bulletin board");
   board_info[21]=new  board_info_type
     (1133, 0, 0, LEVEL_GOD+1, MAX_BOARD_MESSAGES, "magiic",
      "Magus IC bulletin board");
   board_info[22]=new board_info_type
     (1134, LEVEL_GOD, LEVEL_GOD, LEVEL_GRGOD, MAX_BOARD_MESSAGES, "coders",
      "Loman/Fingolfin Code Board");
   board_info[23]=new board_info_type
     (1135, LEVEL_GOD, LEVEL_GOD, LEVEL_GRGOD, MAX_BOARD_MESSAGES, "Alkar",
      "Alkar's board");
   mail_board = new mail_info_type
     (0, 0, 0, 0, MAX_MAIL_MESSAGES, "mail", "Mail Board");
   //   board_info[8] = mail_board;
   //   printf("passed mail_board creation\n");
   tmp_news = news_board_number;
   news_board_number = -1;
   for (i = 0; i < NUM_OF_BOARDS; i++) {
     if(G_VNUM(board_info[i]) == tmp_news ) news_board_number = i;
   }
...
SPECIAL(gen_board)
{
  ...
  if(!wtl || (callflag != SPECIAL_COMMAND)) return 0;
  
  if ((wtl->cmd != CMD_WRITE) && 
      (wtl->cmd != CMD_LOOK) && 
      (wtl->cmd != CMD_READ) && 
      (wtl->cmd != CMD_EXAMINE) &&
      (wtl->cmd != CMD_REMOVE) && 
      (wtl->cmd != CMD_NEXT) && 
      (wtl->cmd != CMD_SEND) && 
      (wtl->subcmd != SCMD_NEWS) &&
      (wtl->subcmd!=SCMD_MAIL))
    return 0;

  cmd = wtl->cmd;
  if(wtl->targ1.type == TARGET_TEXT) arg1 = wtl->targ1.ptr.text->text;
  else arg1 = arg;
  ...
  for(tmp = 0; tmp<29 && arg[tmp]>' '; tmp++) argb[tmp] = arg[tmp];
  argb[tmp] = 0;
  arg1 = arg + tmp;
  ...
  if(wtl->subcmd == SCMD_MAIL){
    ...
    else if(*argb)  cmd = CMD_READ;
    else {
      cmd = CMD_LOOK;
      //      printf("news: argument is:%s.\n",arg);
      if(strlen(argb)>20) argb[20]=0;
      sprintf(argb,"board %s\n",arg);
      arg1 = argb;
    }
   }
  ...
  case CMD_LOOK:
    ...
    break;
  case CMD_EXAMINE:
    if(board->is_changed) board->save_board();
    //    board->is_changed = 0;
...
void
board_info_type::write_message(struct char_data *ch, char *arg, int num)
{
  char *tmstr;
  int len;
  long ct;
  ...
  if((strlen(arg) >= 3) && (!strncmp(arg,"IMM",3)) &&
...
int
board_info_type::approve_msg(char_data *ch, board_msginfo *msg, int cur_num, int *num) {
  int len;
  *num = msg->msg_num;
  len = strlen(msg->heading);
  if(tmp_allflag || (!tmp_allflag && (*num > cur_num))){
    if((len < 30) || (strncmp(msg->heading+27, "IMM", 3)) ||
       (GET_LEVEL(ch) >= LEVEL_IMMORT))
      return 1;
    else
      return 0;
...
int board_info_type::count_msg(char_data * ch, int cur_num){
  ...
  for(tmp=0, count=0; tmp<num_of_msgs; tmp++){
    if(approve_msg(ch, msg_index + tmp, cur_num, &tmp2)) count++;
  }
				    ...
				    char *arg, int allflag)
{
  ...
  if (!*tmp || !isname(tmp, "board bulletin"))
    return 0;
  if(*arg1 && !strcmp("all",arg1)) allflag = 1;
  ...
  tmp_allflag = allflag;
  if(allflag) cur = -1;
  else cur = MSG_CURMSG(ch);
  show_num = 0;
   count = count_msg(ch, cur);
   //   act("$n studies the board.", TRUE, ch, 0, 0, TO_ROOM);
   if((RNUM>0) &&
     (obj_proto[RNUM].action_description))
   ...
   else
    {
      ...
      for (i = 0; i < num_of_msgs; i++) {
	   for (d = descriptor_list; d; d = d->next)
	     chng_mark |=
	       (!d->connected && d->str == &(msg_storage[MSG_SLOTNUM(i)]));
...
int     board_info_type::select_msg(int msg, int softflag){
  int tmp;
  if( msg <= 0) msg = msg_msgnum(0);
  for(tmp = 0; tmp < num_of_msgs; tmp ++){
    if(!softflag){
      if(msg_msgnum(tmp) == msg) break;
    }
    else
      if(msg_msgnum(tmp) >= msg) break;
  }
				     ...
				     char *arg, int nextflag)
{
   ...
   if(*arg && !strncmp("next",arg,tmp)){
     ...
     if((!*arg && !nextflag) || (*arg &&!isdigit(*arg))){
   ...
   if(nextflag){
     if(!*arg){
       msg = MSG_CURMSG(ch);
       if(nextflag == 2) msg--; //"read last", to get the same message again.
       looseflag = 1;
     }
     else{
       msg = atoi(arg);
   ...
   for(tmp = 0, show_num = 0; tmp <num_of_msgs; tmp++)
     if(approve_msg(ch, msg_index + tmp, 0, &show_num))
       if((!looseflag && show_num == msg) ||
	  (looseflag && show_num > msg)) break;
   ...
   */
   ...
   if(msg >= 0 && nextflag) 
     MSG_CURMSG(ch) = show_num;
       ind = msg;// - 1;
   if (MSG_SLOTNUM( ind) < 0 || 
       MSG_SLOTNUM( ind) >= INDEX_SIZE) {
...
//   strcat(ht_name, ".index");
   ...
   for (i = 0; i < num_of_msgs; i++) {
     ...
     if (MSG_SLOTNUM(i) < 0 || 
	 MSG_SLOTNUM(i) >= INDEX_SIZE || 
	 (!(tmp2 = msg_storage[MSG_SLOTNUM(i)])))
...
void mail_info_type::write_message(struct char_data *ch, char * arg, int num){
  ...
  *(tmstr + strlen(tmstr) - 1) = '\0';
  ...
  do{
    for(i = 0; i < 99 && arg[i] > ' ' && arg[i]!=':'; i++) name[i] = arg[i];
    name[i] = 0;
    arg += i;
