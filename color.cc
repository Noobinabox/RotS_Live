...
void
convert_old_colormask(struct char_file_u *ch)
{
  int i;
  ...
  for (i = 0; i < 10; ++i)
    ch->profs.colors[i] =
      ch->profs.color_mask >> (i * 3) & 7;
}



char
get_colornum(struct char_data *ch, int col)
{
  if (!ch) 
    return 0;
...
void
set_colornum(struct char_data *ch, int col, int value)
{
  if (!ch || !ch->profs) 
    return;
 ...
 */
void
set_colors_default(struct char_data *ch)
{
