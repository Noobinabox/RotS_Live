...
int
other_side_num(int ch_race, int i_race)
{
...
recount_light_room(int room)
{
  ...
  for(tmpch = world[room].people; tmpch; tmpch = tmpch->next_in_room)
	...
	if(tmpch->equipment[tmp]->obj_flags.type_flag == ITEM_LIGHT)
	  if((tmpch->equipment[tmp]->obj_flags.value[2] != 0) &&
	     (tmpch->equipment[tmp]->obj_flags.value[3] != 0))
	    count++;
...
void
affect_modify_room(struct room_data *room, byte loc, int mod, 
		   long bitv, char  add)
{  
...
affect_modify(struct char_data *ch, byte loc, int mod, long bitv, char add)
{
  ...
  case APPLY_SPELL:
    ...
    if(add)
      skills[tmp].spell_pointer(tmp2, ch, "", SPELL_TYPE_SPELL,
				ch, 0, 0, 1);
    else
      skills[tmp].spell_pointer(0, ch, "", SPELL_TYPE_ANTI,
				ch, 0, 0, 1);
    break;
...
void	affect_to_room( struct room_data *room, struct affected_type *af)
{
   ...
   for(tmpaf = room->affected; tmpaf; tmpaf = tmpaf->next)
     if(!IS_SET(tmpaf->bitvector, PERMAFFECT)) perms_only = 0;
...
void
affect_remove_notify(struct char_data *ch, struct affected_type *af)
{
 ...
 */
void
affect_from_char_notify(struct char_data *ch, byte skill)
{
  extern char *spell_wear_off_msg[];
...
void
affect_from_char(struct char_data *ch, byte skill)
{
...
char avg_dur, char avg_mod )
{
   ...
   for (hjp = ch->affected; !found && hjp; hjp = hjp->next) {
	...
	//	    af->duration /= 2;
	  ...
	  if(((af->modifier >= 0) && (af->modifier < hjp->modifier)) ||
	    ((af->modifier >= 0) && (af->modifier < hjp->modifier)))
...
void	char_from_room(struct char_data *ch)
{
   ...
   for(tmp = 0; tmp < MAX_WEAR; tmp++)
	 ...
	 if (ch->equipment[tmp]->obj_flags.value[2] && 
	     (ch->equipment[tmp]->obj_flags.value[3])) /* Light is ON */
...
void
char_to_room(struct char_data *ch, int room)
{
  struct char_data *tmpch;
  int tmp;
...
void	obj_to_char(struct obj_data *object, struct char_data *ch)
{
   ...
   if(IS_RIDING(object->carried_by))
     IS_CARRYING_W(object->carried_by->mount_data.mount) += 
...
void	obj_from_char(struct obj_data *object)
{
   ...
   if (object->carried_by->carrying == object){   /* head of list */
	...
	for(i=0; i<MAX_WEAR; i++)
	  if(object->carried_by->equipment[i] == object) break;
	if(i<MAX_WEAR)
...
struct obj_data *unequip_char(struct char_data *ch, int pos)
{
  ...
  obj = ch->equipment[pos];
   ...
   if((pos == HOLD) && !CAN_WEAR(obj, ITEM_HOLD))return obj;
...
//       affect_remove(ch, ch->affected);
   ...
   if(!k2) waiting_list = ch->delay.next;
	 else k2->delay.next = ch->delay.next;
   ...
   if (ch->desc) {
     ...
     else save_char(ch, (new_room < 0)?
	       ((was_in == NOWHERE)?-1:world[was_in].number):new_room, 0);
   }
   
   if (IS_NPC(ch)) {
     if (ch->nr > -1){ /* if mobile */
       mob_index[ch->nr].number--;
       if(mob_index[ch->nr].number < 0) mob_index[ch->nr].number = 0;
     }
     if(GET_LOADLINE(ch)){
...
int
keyword_matches_char(struct char_data *ch, struct char_data *vict, char *keyword)
{
	int check;
   ...
   *tar_obj = 0;
   ...
   if (IS_SET(bitvector, FIND_OBJ_EQUIP)) {
     for (found = FALSE, i = 0; i < MAX_WEAR && !found; i++){
       if(namelen > 2)
	 tmpfound =  (ch->equipment[i] && 
		      strn_cmp(name, ch->equipment[i]->name,namelen) == 0);
       else
       tmpfound = (ch->equipment[i] && 
...
char *
money_message(int sum, int mode)
{
...
int can_swim(struct char_data * ch){
  ...
  if(IS_NPC(ch))
    if(IS_SET(ch->specials2.act, MOB_CAN_SWIM) || IS_AFFECTED(ch, AFF_FLYING) || IS_SHADOW(ch)) return TRUE;

  if(IS_AFFECTED(ch, AFF_SWIM)) return TRUE;
  
  if(!IS_NPC(ch) && GET_SKILL(ch, SKILL_SWIM) > 0) return TRUE;
 
  for(tmpobj = ch->carrying; tmpobj; tmpobj = tmpobj->next_content)
    if (tmpobj->obj_flags.type_flag == ITEM_BOAT) return TRUE;

  for(tmp = 0; tmp < MAX_WEAR; tmp++)
    if(ch->equipment[tmp])
    if ((ch->equipment[tmp])->obj_flags.type_flag == ITEM_BOAT) return TRUE;
...
void stop_riding(struct char_data * ch){
  ...
  while(IS_RIDDEN(ch)) stop_riding(ch->mount_data.rider);
    
  tmpch = 0;

  if(!IS_RIDING(ch)) return;
  ...
  if(char_exists(ch->mount_data.mount_number)){
    ...
    if((mount)->mount_data.rider == ch){
      tmpch = ch;
      (mount)->mount_data.rider = 
	ch->mount_data.next_rider;
      (mount)->mount_data.rider_number = 
	ch->mount_data.next_rider_number;
    }
    else{
	...
	if(tmpch->mount_data.next_rider == ch){
	  tmpch->mount_data.next_rider = 
	    ch->mount_data.next_rider;
	  tmpch->mount_data.next_rider_number = 
	    ch->mount_data.next_rider_number;
	  break;
	}
      }
...
void stop_riding_all(char_data * mount){
  ...
  for(tmpch = character_list; tmpch; tmpch = tmpch->next)
    if(tmpch->mount_data.mount == mount) stop_riding(tmpch);
}
 ...
 */
int
report_zone_power(struct char_data *ch)
{
  struct zone_data *z;
  ...
  if(RACE_GOOD(ch)) {
    if(((z->dark_power > char_power(z->level) * 3/2) && 
	(z->dark_power > z->white_power * 3/2))) 
      return -1;
    if(((z->magi_power > char_power(z->level) * 3/2) && 
	(z->magi_power > z->white_power * 3/2))) 
      return -1;
  }
  
  else if(RACE_EVIL(ch)) {
    if(((z->white_power > char_power(z->level) * 4) &&
	(z->white_power > z->dark_power * 4))) 
      return 1;
    if(((z->magi_power > char_power(z->level) * 4) && 
	(z->magi_power > z->dark_power * 4))) 
      return -1;
  }

  else if(RACE_MAGI(ch)) {
    if(((z->white_power > char_power(z->level) * 4) &&
	(z->white_power > z->magi_power * 4))) 
      return 1;
    if(((z->dark_power > char_power(z->level) * 4) && 
	(z->dark_power > z->magi_power * 4))) 
      return -1;
  }
