...
void
report_wrong_position(struct char_data *ch)
{
  switch (GET_POS(ch)) {
  case POSITION_DEAD:
 ...
 */
void
report_wrong_target(struct char_data *ch, int mask, char has_arg)
{
...
int
target_check_one(struct char_data *ch, int mask, struct target_data *t1)
  /*
...
int
target_check(struct char_data *ch, int cmd, struct target_data *t1,
...
void
replace_aliases(struct char_data *ch, char *line)
{
  ...
  if(ch->specials.alias) {
    for(list = ch->specials.alias; list; list = list->next) {
      ...
      if(!strncmp(line+begin, list->keyword, tmp) &&
	 *(line+begin+tmp) <= ' ')
	break;
    }
    if(list) {
...
int
activate_obj_special(struct obj_data *host, struct char_data *ch, int cmd,
		     char *arg, int callflag, struct waiting_type *wtl)
{
  SPECIAL(*tmpfunc);
	...
	struct waiting_type * wtl, int in_room)
{
  ...
  if((callflag == SPECIAL_TARGET) ||
	...
	if((*obj_index[wtl->targ2.ptr.obj->item_number].func)
	   (wtl->targ2.ptr.ch, ch, cmd, arg, callflag,wtl))
	  return 1;
      break;
...
assign_command_pointers(void)
{
  ...
  COMMANDO(106, POSITION_RESTING,  do_consider,  0, TRUE, 0,
	   FULL_TARGET, FULL_TARGET, CMD_MASK_NO_UNHIDE);
  COMMANDO(107, POSITION_RESTING,  do_group,     1, TRUE, 0,
	   FULL_TARGET, FULL_TARGET, 0);
  ...
  COMMANDO(229, POSITION_FIGHTING,    do_shoot,      0, TRUE, 0,
     TAR_NONE_OK | TAR_CHAR_ROOM | TAR_FIGHT_VICT, TAR_IGNORE, CMD_MASK_NO_UNHIDE);
}
...
int
new_player_select(struct descriptor_data *d, char *arg)
  /*
...
void
introduce_char(struct descriptor_data *d)
{
  FILE *fp;
