...
format_string(char *str)
{
  ...
  linecount = 3;
  strcpy(buf,"   "); bufscan = 3;      // paragraph indentation
  ...
  while(strscan < len) {
    ...
    if(linecount + spacelen + wordlen < screen_width) {
      for(tmp = 0; tmp < spacelen; tmp++) buf[bufscan++] = ' ';
      strscan += spacelen;
      linecount += spacelen;
    }
    else {
...
int
replace_pattern(descriptor_data *d, char *pattern, char *new_pattern)
{
  ...
  for(tmp = 0; tmp < len; tmp++){
    if(!strncmp(*d->str + tmp, pattern, pat_len)){
      ...
      bufscan += new_len;
      if(tmp < d->cur_str) d->cur_str = d->cur_str + new_len - pat_len;
...
void
string_add_init(struct descriptor_data *d, char **str)
{
  char *tmpstr;
  int tmp;
...
void
string_add_finish(struct descriptor_data *d)
  /*** Our writer has decided to stop writing. ***/
{
 if(!d->str)
   ...
   if(strlen(*d->str) >= MAX_STRING_LENGTH) 
...
void
quad_arg(char *arg, int *type, char *name, int *field, char *string)
{
