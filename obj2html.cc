...
struct obj2html_type weapon_subtypes[] = {
  { "Whips and Flails", weapon_keywords[0],
    (1 << (TYPE_WHIP - TYPE_HIT)) |
    (1 << (TYPE_FLAIL - TYPE_HIT)),
  ...
  { "Concussion Weapons", weapon_keywords[3],
    (1 << (TYPE_CRUSH - TYPE_HIT)) |
    (1 << (TYPE_BLUDGEON - TYPE_HIT)) |
    (1 << (TYPE_SMITE - TYPE_HIT)),
 ...
 */
void
obj2html_newtable(struct char_data *ch, FILE *f,
 ...
 */
void
obj2html_endtable(struct char_data *ch, FILE *f,
...
int
obj2html(FILE *f, struct obj_data *o)
{
  int i, found;
  ...
  fprintf(f, "     <TR ALIGN=RIGHT%s>\n",
	  IS_SET(o->obj_flags.extra_flags, ITEM_MAGIC) ? 
	  " BGCOLOR=33033" : "");
  fprintf(f, "         <TD> %d </TD>\n",
  ...
  case ITEM_WEAPON:
    ...
    fprintf(f, "          <TD> %d </TD>\n", o->obj_flags.value[2]);
    fprintf(f, "          <TD> %s </TD>\n",
	    attack_hit_text[weapon_hit_type(o->obj_flags.value[3]) - 
			    TYPE_HIT]);
  ...
  fprintf(f, "         <TD> %d </TD>\n", o->obj_flags.cost);
  fprintf(f, "         <TD> %d </TD>\n", o->obj_flags.level);
  if (o->obj_flags.material >= 0 &&
      o->obj_flags.material < num_of_object_materials)
    fprintf(f, "         <TD> %s </TD>\n",
 ...
 */
int
check_keywords(struct obj2html_type *list, char *arg)
{
  int i, j;
  int types_selected;
  char *c;
...
void
build_output(struct obj2html_type *list, struct obj_data *o)
{
  int i, j;
  ...
  for (i = 0; list[i].title != NULL; ++i) {
    ...
    if (IS_SET(list[i].valid_types, list[i].get_typemask(o)) || 
	(dump_all && IS_SET(list[i].typemask, list[i].get_typemask(o)))) {
	...
	for (j = 0; j < list[i].num_to_output; ++j)
	  if (o->obj_flags.level >
	      list[i].to_output[j]->obj_flags.level)
	    break;
 ...
 */
void
dump_output(struct char_data *ch, FILE *f, struct obj2html_type *list)
{
  int i, j, vnum;
 ...
 */
void
obj2html_clear(struct obj2html_type *list)
{
  int i;
 ...
 */
FILE *
obj2html_finish(struct char_data *ch, FILE *f)
{
  fprintf(f, "</BODY>\n"
	  "</HTML>\n");
  fclose(f);
