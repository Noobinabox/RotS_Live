...
int get_permission(int zonnum, struct char_data * ch, int mode){
  ...
  if(num==MAX_ZONES){
    ...
    if(tmpowner->owner==0) {
      SHAPE_PROTO(ch)->permission=perm=(mode)?0:1;
      if(GET_LEVEL(ch) < LEVEL_GOD) SHAPE_PROTO(ch)->permission=perm=0;
      
    }
    else{
      perm=0;
      for(;tmpowner->owner!=0; tmpowner=tmpowner->next)
        if(tmpowner->owner==ch->specials2.idnum) perm=1;
    }
    if(ch->player.level>=LEVEL_GRGOD) perm=1;
...
#define DESCRCHANGE(line,addr)do{\
  if(!IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_SIMPLE_ACTIVE)){\
    ...
    str[0]=0;\
    SHAPE_PROTO(ch)->tmpstr = str_dup(addr);\
...
#define LINECHANGE(line,addr)do{\
...
void shape_center_proto(struct char_data * ch, char * arg){
  ...
  do{                           /* big loop for chains */
    if(SHAPE_PROTO(ch)->editflag==0){
      ...
      sscanf(arg,"%s",str);
      if( (str[0] >='0') && (str[0] <='9') ){
	SHAPE_PROTO(ch)->editflag=atoi(str); /* now switching to integers.. */ 
	str[0]=0;
	if(SHAPE_PROTO(ch)->editflag==48)
	  SHAPE_PROTO(ch)->editflag=0;   /* sorry, it seems necessary */
      }
      else{
	extra_coms_proto(ch,arg);
	return;
    ...
    if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_SIMPLEMODE)&& 
       !IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
...
#define DIGITCHANGEL(line,addr)do{\
      if(!IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_DIGIT_ACTIVE)){\
    ...
    case 5:      DIGITCHANGEL("MOB FLAG NUMBER",mob->specials2.act)
	mob->specials2.act|=MOB_ISNPC;
	if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	  SHAPE_PROTO(ch)->editflag=proto_chain[5];
      break;
    case 6:      DIGITCHANGEL("'AFFECTED' NUMBER",mob->specials.affected_by);
	if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	  SHAPE_PROTO(ch)->editflag=proto_chain[6];
      break;
    case 7:      DIGITCHANGE("ALIGNMENT",mob->specials2.alignment)
	if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	  SHAPE_PROTO(ch)->editflag=proto_chain[7];
      break;  
/*-----------here go new mobs' features...----------*/
	...
	if(3!=sscanf(arg,"%d %d %d",&tmp,&tmp1,&tmp2)){
      ...
      SHAPE_PROTO(ch)->editflag=0;
	if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	  SHAPE_PROTO(ch)->editflag=proto_chain[9];
      break;
    case 10:
      if(!IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_DIGIT_ACTIVE)){
    ...
    shape_standup(ch,SHAPE_PROTO(ch)->position);
      ...
      SHAPE_PROTO(ch)->editflag=0;
	if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	  SHAPE_PROTO(ch)->editflag=proto_chain[10];
      break;
    case 11:      DIGITCHANGE("DAMAGE",mob->points.damage)
	if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	  SHAPE_PROTO(ch)->editflag=proto_chain[11];
      break;
    case 12:      DIGITCHANGE("ENERGY REGEN",mob->points.ENE_regen)
	if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	  SHAPE_PROTO(ch)->editflag=proto_chain[12];
      break;
    case 13:      DIGITCHANGE("GOLD",mob->points.gold)
	if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	  SHAPE_PROTO(ch)->editflag=proto_chain[13];
	break;
    case 14:      DIGITCHANGE("XP",mob->points.exp)
	if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	  SHAPE_PROTO(ch)->editflag=proto_chain[14];
	break;
/* case 15: here will owner be? */
    case 16:      DIGITCHANGE("POSITION, use 'help shape mob position' for values",mob->specials.position)
	if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	  SHAPE_PROTO(ch)->editflag=proto_chain[16];
      break;
    case 17:      DIGITCHANGE("default POSITION, use 'help shape mob position' for values",mob->specials.default_pos)
	if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	  SHAPE_PROTO(ch)->editflag=proto_chain[17];
      break;
    case 18:      LINECHANGE("SEX (n,m,f)",tmpptr);
	...
	if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	  SHAPE_PROTO(ch)->editflag=proto_chain[18];
      break;
    case 19:      DIGITCHANGE("RACE",mob->player.race)
	if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	  SHAPE_PROTO(ch)->editflag=proto_chain[19];
      break;
    case 20:      DIGITCHANGEL("Race AGGRESSION",mob->specials2.pref)
	if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	  SHAPE_PROTO(ch)->editflag=proto_chain[20];
      break;
    case 21:      DIGITCHANGE("WEIGHT",mob->player.weight)
	if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	  SHAPE_PROTO(ch)->editflag=proto_chain[21];
      break;
    case 22:      DIGITCHANGE("HEIGHT",mob->player.height)
	if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	  SHAPE_PROTO(ch)->editflag=proto_chain[22];
      break;
/* Here go 'M' fields */
    case 23:      DIGITCHANGE("PROF",mob->player.prof)
	if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	  SHAPE_PROTO(ch)->editflag=proto_chain[23];
      break;

    case 24:      DIGITCHANGE("MANA",mob->abilities.mana)
	if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	  SHAPE_PROTO(ch)->editflag=proto_chain[24];
      break;	
    case 25:      DIGITCHANGE("MOVEPOINTS",mob->abilities.move)
	if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	  SHAPE_PROTO(ch)->editflag=proto_chain[25];
      break;
    case 26:      DIGITCHANGE("BODYTYPE",mob->player.bodytype)
	if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	  SHAPE_PROTO(ch)->editflag=proto_chain[26];
      break;
    case 27:      DIGITCHANGE("SAVING THROW",mob->specials2.saving_throw)
	if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	  SHAPE_PROTO(ch)->editflag=proto_chain[27];
      break;
    case 28:
      if(!IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_DIGIT_ACTIVE)){
	...
	ch->specials.prompt_number=5;
      SHAPE_PROTO(ch)->editflag=0;
	if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	  SHAPE_PROTO(ch)->editflag=proto_chain[28];
      break;
    case 29:      DIGITCHANGE("PROGRAM NUMBER",mob->specials.store_prog_number)
	if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	  SHAPE_PROTO(ch)->editflag=proto_chain[29];
      break;
    case 30:      DIGITCHANGE("LANGUAGE",mob->player.language)
	if(mob->player.language > language_number)
	  mob->player.language = language_number;

	if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	  SHAPE_PROTO(ch)->editflag=proto_chain[30];
      break;
    case 31:      DIGITCHANGE("BUTCHER ITEM",mob->specials.butcher_item)

	if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	  SHAPE_PROTO(ch)->editflag=proto_chain[31];
      break;
    case 32:      DIGITCHANGE("PERCEPTION",mob->specials2.perception)

	if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	  SHAPE_PROTO(ch)->editflag=proto_chain[32];
      break;
    case 33:   LINECHANGE("DEATH CRY (to the room)", mob->player.death_cry)
      if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	SHAPE_PROTO(ch)->editflag=proto_chain[33];
      break;
    case 34:   LINECHANGE("DEATH CRY (to other rooms)", mob->player.death_cry2)
      if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	SHAPE_PROTO(ch)->editflag=proto_chain[34];
      break;
    case 35:   DIGITCHANGE("CORPSE VIRT. NUMBER", mob->player.corpse_num)
      if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	SHAPE_PROTO(ch)->editflag=proto_chain[35];
      break;
    case 36:      DIGITCHANGE("RESISTANCE BITVECTOR",mob->specials.resistance)

	if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	  SHAPE_PROTO(ch)->editflag=proto_chain[36];
      break;
    case 37:      DIGITCHANGE("VULNERABILITY BITVECTOR",mob->specials.vulnerability)

	if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	  SHAPE_PROTO(ch)->editflag=proto_chain[37];
      break;
	case 38:      DIGITCHANGE("SCRIPT NUMBER",mob->specials.script_number)
	
	if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	  SHAPE_PROTO(ch)->editflag=proto_chain[38];
      break;

  case 39: DIGITCHANGE("ROLEPLAY FLAG",mob->specials2.rp_flag)
...
#undef DIGITCHANGE
    case 48:
      ...
      tmpptr[0]='N'; tmpptr[1]=0;
      LINECHANGE("choice [Y/N]", tmpptr);
      if((tmpptr[0]!='y') &&(tmpptr[0]!='Y')) {
	SHAPE_PROTO(ch)->editflag=0;
      }
      else {
	SHAPE_PROTO(ch)->editflag=proto_chain[48];
	REMOVE_BIT(SHAPE_PROTO(ch)->flags,SHAPE_SIMPLE_ACTIVE);
	send_to_char("Continuing in extended mode.\n\r",ch);
      ...
      tmpptr=0;
      break;
    case 49: send_to_char("You invoked mobile creation sequence.\n\r",ch);
 	  SHAPE_PROTO(ch)->editflag=proto_chain[49];
	...
	list_proto(ch,mob);
      SHAPE_PROTO(ch)->editflag=0;
	if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_CHAIN))
	  SHAPE_PROTO(ch)->editflag=proto_chain[50];
      break;
    default:
      if(IS_SET(SHAPE_PROTO(ch)->flags,SHAPE_SIMPLEMODE))
	list_simple_help(ch);
...
int
replace_proto(struct char_data *ch,char *arg)
{
  char str[255];
  char *f_from, *f_old;
  char c;
...
void implement_proto(struct char_data * ch){
  ...
  */
	...
	language_skills[SHAPE_PROTO(ch)->proto->player.language-1];
    else 
      proto->player.language = 0;
    if(!IS_SET(proto->specials2.act, MOB_SPEC))
      proto->specials.store_prog_number = 
	real_program(proto->specials.store_prog_number);
    else
...
ACMD(do_shape)
{
  ...
  if(1!=sscanf(argument,"%s",str)) key=0;
  else{
    stlen+=i=strlen(str);
    if(!strncmp("mobile",str,i)) key=SHAPE_PROTOS;
    else if(!strncmp("object",str,i)) key=SHAPE_OBJECTS;
    else if(!strncmp("room",str,i)) key=SHAPE_ROOMS;
    else if(!strncmp("zone",str,i)) key=SHAPE_ZONES;
    else if(!strncmp("program",str,i)) key=SHAPE_MUDLLES;
    else if(!strncmp("script",str,i)) key=SHAPE_SCRIPTS;
    else if(!strcmp ("recalc_mobile", str) 
	    && (GET_LEVEL(ch) == LEVEL_IMPL)) key = SHAPE_RECALC_ALL;
    else if(!strcmp ("master_mobile", str) 
	    && (GET_LEVEL(ch) >= LEVEL_GRGOD)) key = SHAPE_MASTER_MOBILE;
    else if(!strcmp ("master_object", str) 
	    && (GET_LEVEL(ch) >= LEVEL_GRGOD)) key = SHAPE_MASTER_OBJECT;
...
send_to_char("Unrecognized parameter. Choose 'object', 'mob', 'room', 'zone' or script\n\r",ch);
  ...
  if (key && !ch->temp){
    ...
    if(strlen(tmp)&& !strncmp("new",tmp,strlen(tmp))){
      for(stlen=0;argument[stlen]<=' ';stlen++);
      for(       ;argument[stlen]>' ';stlen++); /*room*/
      for(       ;argument[stlen]<=' ';stlen++);
      for(       ;argument[stlen]>' ';stlen++); /*new*/
      sprintf(str,"new %s\n\r",argument+stlen);
      newflag=1;
    }
    else{
    ...
    case SHAPE_OBJECTS:
      ...
      SHAPE_OBJECT(ch)->act=SHAPE_OBJECTS;
      SHAPE_OBJECT(ch)->flags=0;
      ...
      SHAPE_OBJECT(ch)->position=ch->specials.position;
      SHAPE_OBJECT(ch)->number=-1;
      SET_BIT(PRF_FLAGS(ch),PRF_DISPTEXT);
    ...
    case SHAPE_ROOMS:
      ...
      SHAPE_ROOM(ch)->act=SHAPE_ROOMS;
      SHAPE_ROOM(ch)->flags=0;
      ...
      SHAPE_ROOM(ch)->position=ch->specials.position;
    ...
    case SHAPE_ZONES:
      ...
      SHAPE_ZONE(ch)->act=SHAPE_ZONES;
      SHAPE_ZONE(ch)->flags=0;
      //      SHAPE_ZONE(ch)->tmpstr=(char *)calloc(1,1);
      CREATE(SHAPE_ZONE(ch)->tmpstr, char, 1);
      SHAPE_ZONE(ch)->mask.command='*';
      SHAPE_ZONE(ch)->mask.if_flag=-1;
      SHAPE_ZONE(ch)->mask.arg1=-1;
      SHAPE_ZONE(ch)->mask.arg2=-1;
      SHAPE_ZONE(ch)->mask.arg3=-1;
      SHAPE_ZONE(ch)->mask.arg4=-1;
      SHAPE_ZONE(ch)->mask.arg5=-1;
      SHAPE_ZONE(ch)->mask.arg6=-1;
      SHAPE_ZONE(ch)->mask.arg7=-1;
      SHAPE_ZONE(ch)->cur_room = 0;
      SHAPE_ZONE(ch)->procedure=SHAPE_EDIT;
      SET_BIT(PRF_FLAGS(ch),PRF_DISPTEXT);
	...
	case SHAPE_SCRIPTS:
	  ...
	  SHAPE_SCRIPT(ch)->act = SHAPE_SCRIPTS;
	  SHAPE_SCRIPT(ch)->flags = 0;
	  SHAPE_SCRIPT(ch)->procedure = SHAPE_EDIT;
	  SET_BIT(PRF_FLAGS(ch),PRF_DISPTEXT);
      ch->specials.prompt_number=9;
      send_to_char("You start shaping a script.\n\r",ch);
      SHAPE_SCRIPT(ch)->position=ch->specials.position;
	...
	break;
      ...
      SHAPE_MUDLLE(ch)->act=SHAPE_MUDLLES;
      SHAPE_MUDLLE(ch)->prog_num = -1;
      SET_BIT(PRF_FLAGS(ch),PRF_DISPTEXT);
