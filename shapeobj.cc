...
void new_obj(struct char_data * ch){
  ...
  //    bzero((char *)(SHAPE_OBJECT(ch)->object),sizeof(struct obj_data));
    ...
    SHAPE_OBJECT(ch)->basenum=-1;
    //    SHAPE_OBJECT(ch)->object->name=(char *)calloc(6,1);
    //    strcpy(SHAPE_OBJECT(ch)->object->name,"gizmo");
    SHAPE_OBJECT(ch)->object->name = str_dup("gizmo");
    //    SHAPE_OBJECT(ch)->object->short_description=(char *)calloc(6,1);
    //    strcpy(SHAPE_OBJECT(ch)->object->short_description,"gizmo");
    SHAPE_OBJECT(ch)->object->short_description=str_dup("gizmo");
    //    SHAPE_OBJECT(ch)->object->description=(char *)calloc(8,1);
    //    strcpy(SHAPE_OBJECT(ch)->object->description,"gizmo");
    SHAPE_OBJECT(ch)->object->description = str_dup("gizmo");
    //    SHAPE_OBJECT(ch)->object->action_description=(char *)calloc(6,1);
    //    strcpy(SHAPE_OBJECT(ch)->object->action_description,"gizmo");
    SHAPE_OBJECT(ch)->object->action_description = str_dup("");
    SHAPE_OBJECT(ch)->object->obj_flags.type_flag=ITEM_TRASH;
    SHAPE_OBJECT(ch)->object->obj_flags.extra_flags=0;
    SHAPE_OBJECT(ch)->object->obj_flags.wear_flags=ITEM_TAKE;
    for(i=0;i<5;i++) SHAPE_OBJECT(ch)->object->obj_flags.value[i]=0;
    SHAPE_OBJECT(ch)->object->obj_flags.weight=0;
    SHAPE_OBJECT(ch)->object->obj_flags.cost=0;
    SHAPE_OBJECT(ch)->object->obj_flags.cost_per_day=0;
    SHAPE_OBJECT(ch)->object->ex_description=0;
    for(i=0;i<MAX_OBJ_AFFECT;i++){
      SHAPE_OBJECT(ch)->object->affected[i].location=APPLY_NONE;
      SHAPE_OBJECT(ch)->object->affected[i].modifier=0;
    }

    SHAPE_OBJECT(ch)->object->carried_by=0;
    SHAPE_OBJECT(ch)->object->in_obj=0;
    SHAPE_OBJECT(ch)->object->contains=0;
    SHAPE_OBJECT(ch)->object->in_room=ch->in_room;
}
...
#define SUBST(x) CREATE(real->x, char, strlen(curr->x)+1); strcpy(real->x,curr->x);
...
void implement_object(struct char_data * ch){
 ...
 // int tmp;
  ...
  curr=SHAPE_OBJECT(ch)->object;
    SUBST(name); SUBST(short_description);
  SUBST(description); SUBST(action_description);
...
#define DESCRCHANGE(line,addr)do{\
  if(!IS_SET(SHAPE_OBJECT(ch)->flags,SHAPE_SIMPLE_ACTIVE)){\
    ...
    str[0]=0;\
    SHAPE_OBJECT(ch)->tmpstr = str_dup(addr);\
...
//  int i,i1;
  ...
  do{              /* big chain loop */
    if(SHAPE_PROTO(ch)->editflag==0){
      ...
      sscanf(arg,"%s",str);
      if( (str[0] >='0') && (str[0] <='9') ){
	SHAPE_OBJECT(ch)->editflag=atoi(str); /* now switching to integers.. */ 
	str[0]=0;
      }
      else{
	extra_coms_obj(ch,arg);
  ...
  if(current_descr)
    while(current_descr->next) current_descr=current_descr->next;
	...
	if(IS_SET(SHAPE_OBJECT(ch)->flags,SHAPE_CHAIN))
	  SHAPE_OBJECT(ch)->editflag=obj_chain[1];
	else SHAPE_OBJECT(ch)->editflag = 0;
      break;
	...
	if(IS_SET(SHAPE_OBJECT(ch)->flags,SHAPE_CHAIN))
	  SHAPE_OBJECT(ch)->editflag=obj_chain[2];
	else SHAPE_OBJECT(ch)->editflag = 0;
      break;
    case 3:
	...
	if(IS_SET(SHAPE_OBJECT(ch)->flags,SHAPE_CHAIN))
	  SHAPE_OBJECT(ch)->editflag=obj_chain[3];
	else SHAPE_OBJECT(ch)->editflag = 0;
      break;
    case 4:
      DESCRCHANGE("ACTION DESCRIPTION",SHAPE_OBJECT(ch)->object->action_description)
	if(IS_SET(SHAPE_OBJECT(ch)->flags,SHAPE_CHAIN))
	  SHAPE_OBJECT(ch)->editflag=obj_chain[4];
	else SHAPE_OBJECT(ch)->editflag = 0;
      break;
    case 5:
      if(current_descr){
	...
	current_descr=SHAPE_OBJECT(ch)->object->ex_description;
      ...
      if(IS_SET(SHAPE_OBJECT(ch)->flags,SHAPE_CHAIN))
	SHAPE_OBJECT(ch)->editflag=obj_chain[5];
      else SHAPE_OBJECT(ch)->editflag = 0;
      break;
	...
	if(IS_SET(SHAPE_OBJECT(ch)->flags,SHAPE_CHAIN))
	  SHAPE_OBJECT(ch)->editflag=obj_chain[6];
	else SHAPE_OBJECT(ch)->editflag = 0;
      break;
	...
	if(IS_SET(SHAPE_OBJECT(ch)->flags,SHAPE_CHAIN))
	  SHAPE_OBJECT(ch)->editflag=obj_chain[7];
	else SHAPE_OBJECT(ch)->editflag = 0;
      break;
	...
	if(IS_SET(SHAPE_OBJECT(ch)->flags,SHAPE_CHAIN))
	  SHAPE_OBJECT(ch)->editflag=obj_chain[8];
	else SHAPE_OBJECT(ch)->editflag = 0;
	break;
      }
      while((current_descr->next)->next) current_descr=current_descr->next;
...
#define DIGITCHANGE(line,addr)\
   ...
   ch->specials.prompt_number=3;\
    ...
    case 9:      DIGITCHANGE("TYPE_FLAG NUMBER",obj->obj_flags.type_flag);

	if(IS_SET(SHAPE_OBJECT(ch)->flags,SHAPE_CHAIN))
	  SHAPE_OBJECT(ch)->editflag=obj_chain[9];
	else SHAPE_OBJECT(ch)->editflag = 0;
      break;

    case 10:      DIGITCHANGE("EXTRA_FLAGS NUMBER",obj->obj_flags.extra_flags);

 	if(IS_SET(SHAPE_OBJECT(ch)->flags,SHAPE_CHAIN))
	  SHAPE_OBJECT(ch)->editflag=obj_chain[10];
	else SHAPE_OBJECT(ch)->editflag = 0;
     break;

    case 11:      DIGITCHANGE("WEAR_FLAGS",obj->obj_flags.wear_flags)
 	if(IS_SET(SHAPE_OBJECT(ch)->flags,SHAPE_CHAIN))
	  SHAPE_OBJECT(ch)->editflag=obj_chain[11];
	else SHAPE_OBJECT(ch)->editflag = 0;
     break;  
	...
	shape_standup(ch,SHAPE_OBJECT(ch)->position);
      ...
      if( tmp5>0)obj->obj_flags.value[0]=tmp; 
      if( tmp5>1)obj->obj_flags.value[1]=tmp1;
      if( tmp5>2)obj->obj_flags.value[2]=tmp2;
      if( tmp5>3) obj->obj_flags.value[3]=tmp3;
      if( tmp5>4) obj->obj_flags.value[4]=tmp4;

      shape_standup(ch,SHAPE_OBJECT(ch)->position);
     REMOVE_BIT(SHAPE_OBJECT(ch)->flags,SHAPE_DIGIT_ACTIVE);
      SHAPE_OBJECT(ch)->editflag=0;
	if(IS_SET(SHAPE_OBJECT(ch)->flags,SHAPE_CHAIN))
	  SHAPE_OBJECT(ch)->editflag=obj_chain[12];
	else SHAPE_OBJECT(ch)->editflag = 0;
      break;

    case 13:      DIGITCHANGE("WEIGHT",obj->obj_flags.weight)

	if(IS_SET(SHAPE_OBJECT(ch)->flags,SHAPE_CHAIN))
	  SHAPE_OBJECT(ch)->editflag=obj_chain[13];
	else SHAPE_OBJECT(ch)->editflag = 0;
      break;  

    case 14:      DIGITCHANGE("COST",obj->obj_flags.cost)

	if(IS_SET(SHAPE_OBJECT(ch)->flags,SHAPE_CHAIN))
	  SHAPE_OBJECT(ch)->editflag=obj_chain[14];
	else SHAPE_OBJECT(ch)->editflag = 0;
      break;  

    case 15:      DIGITCHANGE("RENT COST PER HOUR",obj->obj_flags.cost_per_day)

	if(IS_SET(SHAPE_OBJECT(ch)->flags,SHAPE_CHAIN))
	  SHAPE_OBJECT(ch)->editflag=obj_chain[15];
	else SHAPE_OBJECT(ch)->editflag = 0;
      break;  

    case 16:      DIGITCHANGE("LEVEL",obj->obj_flags.level)

	if(IS_SET(SHAPE_OBJECT(ch)->flags,SHAPE_CHAIN))
	  SHAPE_OBJECT(ch)->editflag=obj_chain[16];
	else SHAPE_OBJECT(ch)->editflag = 0;
      break;  

    case 17:      DIGITCHANGE("RARITY",obj->obj_flags.rarity)

	if(IS_SET(SHAPE_OBJECT(ch)->flags,SHAPE_CHAIN))
	  SHAPE_OBJECT(ch)->editflag=obj_chain[17];
	else SHAPE_OBJECT(ch)->editflag = 0;
      break;  

    case 18:      DIGITCHANGE("MATERIAL",obj->obj_flags.material)

	if(IS_SET(SHAPE_OBJECT(ch)->flags,SHAPE_CHAIN))
	  SHAPE_OBJECT(ch)->editflag=obj_chain[18];
	else SHAPE_OBJECT(ch)->editflag = 0;
      break;  
 ...
 while((2==(tmp4=sscanf(arg+tmp3," (%d %d) ",&tmp1,&tmp2)))&& (tmp<MAX_OBJ_AFFECT)){
	  ...
	  tmp++;
	  while((arg[tmp3]!=')')&&(arg[tmp3]!=0)&&(tmp3<255)) tmp3++;
	  tmp3++;
 ...
 /*	  printf("Arg string:%s, tmp=%d tmp4=%d tmp3=%d\n\r",arg+tmp3,tmp,tmp4,tmp3);*/
   ...
   ch->specials.prompt_number=6;
	...
	if(IS_SET(SHAPE_OBJECT(ch)->flags,SHAPE_CHAIN))
	  SHAPE_OBJECT(ch)->editflag=obj_chain[19];
	else SHAPE_OBJECT(ch)->editflag = 0;
	  return;
   ...
   ch->specials.prompt_number=6;
    ...
    case 20:
      ...
      if(IS_SET(SHAPE_OBJECT(ch)->flags, SHAPE_CHAIN))
	SHAPE_OBJECT(ch)->editflag = obj_chain[20];
      else 
	SHAPE_OBJECT(ch)->editflag = 0;
      break;
    ...
    case 21: 
      ...
      if(IS_SET(SHAPE_OBJECT(ch)->flags,SHAPE_CHAIN))
	SHAPE_OBJECT(ch)->editflag=obj_chain[21];
      else 
	SHAPE_OBJECT(ch)->editflag = 0;
      break;
...
#undef DIGITCHANGE
    ...
    case 49: send_to_char("You invoked object creation sequence.\n\r",ch);
 	  SHAPE_OBJECT(ch)->editflag=obj_chain[49];
	  SET_BIT(SHAPE_OBJECT(ch)->flags,SHAPE_CHAIN);
     break;
...
static char str[MAX_STRING_LENGTH];//str1[100];
  ...
  else send_to_char("No extra descriptions for this object\n\r",ch);
  ...
  sprintf(str,"(18) material     :%d (%s)\n\r",obj->obj_flags.material,
	  ((obj->obj_flags.material >= 0) && 
	   (obj->obj_flags.material < num_of_object_materials))?
	  object_materials[obj->obj_flags.material]: "Unknown");
...
int load_object(struct char_data * ch,char * arg){
  ...
  if(GET_IDNUM(ch) == object_master_idnum || GET_IDNUM(ch) == object_master2_idnum)
    SHAPE_OBJECT(ch)->permission = 1;
  else
    SHAPE_OBJECT(ch)->permission=get_permission(number/100, ch);

  tmp=find_obj(f,number);
  if(tmp==-1) {send_to_char(" no object here.\n\r",ch); fclose(f);return -1;}
  ...
  */
  if(tmp>number){
    SHAPE_OBJECT(ch)->number=number;
    SHAPE_OBJECT(ch)->basenum=number;
    new_obj(ch);
...
sprintf(str,"loading object #%d\n\r",tmp);
  ...
  SHAPE_OBJECT(ch)->object->name=0;
  SHAPE_OBJECT(ch)->object->short_description=0;
  SHAPE_OBJECT(ch)->object->description=0;
  SHAPE_OBJECT(ch)->object->action_description=0;




  SHAPE_OBJECT(ch)->basenum=number; /*    ????????     to check    */
  SHAPE_OBJECT(ch)->number=number;
  get_text(f,&(st));
  SHAPE_OBJECT(ch)->object->name=st;st=0;
  get_text(f,&(st));
  SHAPE_OBJECT(ch)->object->short_description=st; st=0;
  get_text(f,&(st));
  SHAPE_OBJECT(ch)->object->description=st;st=0;
  get_text(f,&(st));
  SHAPE_OBJECT(ch)->object->action_description=st;st=0;

  if(1!=fscanf(f,"%d ",&tmp)) printf("Error reading1\n\r");
  SHAPE_OBJECT(ch)->object->obj_flags.type_flag=tmp;
  if(1!=fscanf(f,"%d ",&tmp))printf("Error reading2\n\r");
  SHAPE_OBJECT(ch)->object->obj_flags.extra_flags=tmp;
  if(1!=fscanf(f,"%d ",&tmp))printf("Error reading3\n\r");
  SHAPE_OBJECT(ch)->object->obj_flags.wear_flags=tmp;
  if(1!=fscanf(f,"%d ",&tmp))printf("Error reading4\n\r");
  SHAPE_OBJECT(ch)->object->obj_flags.value[0] = tmp;
  if(1!=fscanf(f,"%d ",&tmp))printf("Error reading5\n\r"); 
  SHAPE_OBJECT(ch)->object->obj_flags.value[1] = tmp;
  if(1!=fscanf(f,"%d ",&tmp))printf("Error reading6\n\r"); 
  SHAPE_OBJECT(ch)->object->obj_flags.value[2] = tmp;
  if(1!=fscanf(f,"%d ",&tmp))printf("Error reading7\n\r"); 
  SHAPE_OBJECT(ch)->object->obj_flags.value[3] = tmp;
  if(1!=fscanf(f,"%d ",&tmp))printf("Error reading8\n\r"); 
  SHAPE_OBJECT(ch)->object->obj_flags.value[4] = tmp;


  if(1!=fscanf(f,"%d ",&tmp))printf("Error reading9\n\r");  
  SHAPE_OBJECT(ch)->object->obj_flags.weight = tmp;
  if(1!=fscanf(f,"%d ",&tmp))printf("Error reading10\n\r");
  SHAPE_OBJECT(ch)->object->obj_flags.cost = tmp;
  if(1!=fscanf(f,"%d ",&tmp))printf("Error reading11\n\r"); 
  SHAPE_OBJECT(ch)->object->obj_flags.cost_per_day = tmp;

  if(5!=fscanf(f,"%d %d %d %d %d",&tmp, &tmp2, &tmp3, &tmp4, &tmp5))
    printf("Error reading12\n\r"); 
  SHAPE_OBJECT(ch)->object->obj_flags.level = tmp;
  SHAPE_OBJECT(ch)->object->obj_flags.rarity = tmp2;
  SHAPE_OBJECT(ch)->object->obj_flags.material = tmp3;
  SHAPE_OBJECT(ch)->object->obj_flags.script_number = tmp4;
  ...
  for (i = 0 ; (i < MAX_OBJ_AFFECT) && (str[0] == 'A') ; i++) {
            ...
            SHAPE_OBJECT(ch)->object->affected[i].location = tmp;
            SHAPE_OBJECT(ch)->object->affected[i].modifier = tmp2;
            fscanf(f, " %s \n\r", str);
         }
...
*/
  ...
  if(check==EOF) {sprintf(str,"no mob #%d in this file\n\r",num);
...
/*  if(3!=sscanf(arg,"%s %s %s",str,f_from,f_old)){
  ...
  do{
    ...
    check=fscanf(f1,"%c",&c);if(check!=EOF)fprintf(f2,"%c",c);
  ...
  if(SHAPE_OBJECT(ch)->procedure==SHAPE_EDIT){
   ...
   send_to_char("you invoked some rhymes from shapeless indefinity...\n\r",ch);
    ...
    do{
      ...
      if(!strncmp(str,"delete",strlen(str))) {comm_key=SHAPE_DELETE;break;}            if(!strncmp(str,"implement",strlen(str))) {comm_key=SHAPE_IMPLEMENT;break;}
...
/*    SHAPE_OBJECT(ch)->procedure=comm_key;*/
  ...
  else comm_key=SHAPE_OBJECT(ch)->procedure;
  switch(comm_key){
  case SHAPE_FREE:
    free_object(ch);      
  ...
  case SHAPE_CREATE:
    ...
    SHAPE_OBJECT(ch)->permission = get_permission(i, ch);
    ...
    SHAPE_OBJECT(ch)->editflag=49;
    shape_center_obj(ch,"");    break;
  ...
  case SHAPE_DELETE:
    if(SHAPE_OBJECT(ch)->procedure!=SHAPE_DELETE){
      ...
      SHAPE_OBJECT(ch)->procedure=SHAPE_DELETE;
      SHAPE_OBJECT(ch)->position=shape_standup(ch,POSITION_SHAPING);
