...
int load_script(struct char_data *ch, char * arg){
  ...
  if ((index = find_script_by_number(number)) == -1){
    ...
    send_to_char(str, ch);
    SHAPE_SCRIPT(ch)->name = 0;
    SHAPE_SCRIPT(ch)->description = 0;
    SHAPE_SCRIPT(ch)->number = number;
	SHAPE_SCRIPT(ch)->index_pos = -1;
	...
	SHAPE_SCRIPT(ch)->root = newscript;
	SHAPE_SCRIPT(ch)->script = newscript;
  } else {
  
	SHAPE_SCRIPT(ch)->index_pos = index;
	...
	SHAPE_SCRIPT(ch)->number = script_table[index].number;
	SHAPE_SCRIPT(ch)->script = 0;
	SHAPE_SCRIPT(ch)->root = 0;
  ...
  SHAPE_SCRIPT(ch)->permission = get_permission(number/100, ch);
  return number;
}
...
void show_command(char_data * ch, script_data * script){
  ...
  case ON_DIE:
	...
	case SCRIPT_DO_SAY:
	  sprintf(buf, "[%d] ACT DO_SAY           ""%s"" (%s)(%s)\n\r", script->number, script->text,
	    get_param_text(script->param[0]), get_param_text(script->param[1]));
	  break;
  ...
  case SCRIPT_DO_WAIT:
	...
	case SCRIPT_DO_YELL:
	  sprintf(buf, "[%d] ACT DO_YELL          ""%s"" %s (%s)\n\r", script->number, script->text,
		get_param_text(script->param[0]), get_param_text(script->param[1]));
	  break;
	...
	case SCRIPT_IF_STR_CONTAINS:
	  sprintf(buf, "[%d] IF_STR_CONTAINS:     does %s contain ""%s""?\n\r", script->number,
	    get_param_text(script->param[0]), script->text);
	  break;
	
	case SCRIPT_IF_STR_EQUAL:
	  sprintf(buf, "[%d] IF_STR_EQUAL:        is %s the same as ""%s""?\n\r", script->number,
	    get_param_text(script->param[0]), script->text);
	  break;
  ...
  case SCRIPT_OBJ_FROM_CHAR:
	...
	case SCRIPT_SEND_TO_CHAR:
	  sprintf(buf, "[%d] MSG SEND_TO_CHAR     ""%s"" to: %s (%s)\n\r", script->number, script->text,
		get_param_text(script->param[0]), get_param_text(script->param[1]));
	  break;
	  
	case SCRIPT_SEND_TO_ROOM:
	  sprintf(buf, "[%d] MSG SEND_TO_ROOM     ""%s"" (%s)(%s)\n\r", script->number, script->text,
	    get_param_text(script->param[0]), get_param_text(script->param[1]));
	  break;
	
	case SCRIPT_SEND_TO_ROOM_X:
	  sprintf(buf, "[%d] MSG SEND_TO_ROOM_X   ""%s"" (%s, %s)\n\r", script->number, script->text,
		get_param_text(script->param[0]), get_param_text(script->param[1]));
	  break;
...
void extra_coms_script(struct char_data * ch, char * argument){
  ...
  else comm_key=SHAPE_SCRIPT(ch)->procedure;
  switch(comm_key){
  case SHAPE_FREE:
    free_script(ch);
  ...
  case SHAPE_CREATE:
    ...
    SHAPE_SCRIPT(ch)->permission = get_permission(zonnum, ch);
  ...
  case SHAPE_DELETE:
    if(SHAPE_SCRIPT(ch)->procedure!= SHAPE_DELETE){
      ...
      SHAPE_SCRIPT(ch)->procedure = SHAPE_DELETE;
...
#define SCRIPTDESCRCHANGE(line,addr)do{\
  if(!IS_SET(SHAPE_SCRIPT(ch)->flags,SHAPE_SIMPLE_ACTIVE)){\
    ...
    str[0]=0;\
    SHAPE_SCRIPT(ch)->tmpstr = str_dup(addr);\
...
#define SCRIPTLINECHANGE(line,addr)do{\
...
#define SCRIPTREALDIGCHANGE(line,addr)do{\
      if(!IS_SET(SHAPE_SCRIPT(ch)->flags,SHAPE_DIGIT_ACTIVE)){\
      ...
      if (!IS_SET(SHAPE_SCRIPT(ch)->flags, SHAPE_DIGIT_ACTIVE)){\
		...
		for (i = 0; i < num; i++) {\
		  while ((arg[tmp2] < '0') && (arg[tmp2] != '-') && (arg[tmp2]))\
			tmp2++;\
		  SHAPE_SCRIPT(ch)->script->param[i] = atoi(arg + tmp2);\
...
if(!IS_SET(SHAPE_SCRIPT(ch)->flags,SHAPE_DIGIT_ACTIVE)) {    \
  ...
  for(i = 0; i < num; i++)                                   \
    if(get_parameter(input[i]))                              \
      SHAPE_SCRIPT(ch)->script->param[i] = get_parameter(input[i]);\
    else                                                     \
      SHAPE_SCRIPT(ch)->script->param[i] = atoi(input[i]);   \
    SHAPE_SCRIPT(ch)->position =                             \
    shape_standup(ch,SHAPE_SCRIPT(ch)->position);            \
    ch->specials.prompt_number = 9;                          \
    REMOVE_BIT(SHAPE_SCRIPT(ch)->flags, SHAPE_DIGIT_ACTIVE); \
    SHAPE_SCRIPT(ch)->editflag = 0;                          \
...
void shape_center_script(struct char_data *ch, char * arg){
  ...
  if(SHAPE_SCRIPT(ch)->editflag == 0){
    sscanf(arg,"%s",str);
    if( (str[0] >= '0') && (str[0] <= '9') ){
	  SHAPE_SCRIPT(ch)->editflag = atoi(str);
	  str[0]=0;
	  if( SHAPE_SCRIPT(ch)->editflag == 0){
		list_help_script(ch);
		return;
  ...
  if(IS_SET(SHAPE_SCRIPT(ch)->flags,SHAPE_CURRFLAG))
    SHAPE_SCRIPT(ch)->cur_room = world[ch->in_room].number;
  ...
  while(SHAPE_SCRIPT(ch)->editflag)  // big loop 
	...
	switch(SHAPE_SCRIPT(ch)->editflag){
	  ...
	  case 3:                            // case 3: Set Command
		...
		SHAPE_SCRIPT(ch)->script->command_type = get_command(st1);
		SHAPE_SCRIPT(ch)->editflag = 4;
      ...
      break;
		  ...
		  case SCRIPT_DO_YELL:
			...
			SHAPE_SCRIPT(ch)->script->param[5] = SHAPE_SCRIPT(ch)->script->param[0];
			SHAPE_SCRIPT(ch)->editflag = 48;
			break;
		  ...
		  case SCRIPT_OBJ_TO_ROOM:
			...
			SHAPE_SCRIPT(ch)->script->param[1] = SHAPE_SCRIPT(ch)->script->param[0];
			SHAPE_SCRIPT(ch)->editflag = 46;
			break;
		  ...
		  case SCRIPT_SET_EXIT_STATE:
			...
			SHAPE_SCRIPT(ch)->script->param[2] = SHAPE_SCRIPT(ch)->script->param[0];
			SHAPE_SCRIPT(ch)->editflag = 43;
			break;
    ...
    SHAPE_SCRIPT(ch)->editflag = 5;
	  ...
	  tmp1 = SHAPE_SCRIPT(ch)->script->number;
		...
		if(!script)
		  ...
		  SHAPE_SCRIPT(ch)->script = script;
		  if (SHAPE_SCRIPT(ch)->cur_room &&(SHAPE_SCRIPT(ch)->cur_room!=script->room)){
			SHAPE_SCRIPT(ch)->cur_room = script->room;
	  ...
	  case 9:   // Case 9: remove command
	    ...
	    if (SHAPE_SCRIPT(ch)->root == SHAPE_SCRIPT(ch)->script){   // ie the first command
	      ...
	      SHAPE_SCRIPT(ch)->root = SHAPE_SCRIPT(ch)->script->next;
	      RELEASE(SHAPE_SCRIPT(ch)->script);
	      SHAPE_SCRIPT(ch)->script = tmpscript;
	    } else {
	      SHAPE_SCRIPT(ch)->script->prev->next = SHAPE_SCRIPT(ch)->script->next;
	      if (SHAPE_SCRIPT(ch)->script->next)
	        SHAPE_SCRIPT(ch)->script->next->prev = SHAPE_SCRIPT(ch)->script->prev;
	      else
	        SHAPE_SCRIPT(ch)->script->next = 0;
	  ...
	  case 10:                           // Case 10: insert new command after current
		...
		SHAPE_SCRIPT(ch)->script->next = script;
		CREATE(script->text, char, 1);
		script->text[0] = 0;
		script->command_type = SCRIPT_COMMAND_NONE;
		SHAPE_SCRIPT(ch)->script = SHAPE_SCRIPT(ch)->script->next;
		renum_commands(SHAPE_SCRIPT(ch)->root);
	  ...
	  case 11:                            // Case 11: insert new command before current
		...
		if(SHAPE_SCRIPT(ch)->script->prev) 
		  ...
		  SHAPE_SCRIPT(ch)->root = script;
		  script->prev = 0;
		}
		SHAPE_SCRIPT(ch)->script->prev = script;
		CREATE(script->text, char, 1);
		script->text[0] = 0;
		SHAPE_SCRIPT(ch)->script = SHAPE_SCRIPT(ch)->script->prev;
		renum_commands(SHAPE_SCRIPT(ch)->root);    
	  ...
	  case 13:                             // Case 13: switch commands
		if(!SHAPE_SCRIPT(ch)->script->next){
		  ...
		  SHAPE_SCRIPT(ch)->script->next = script->next;
		  if (SHAPE_SCRIPT(ch)->script->prev)
			SHAPE_SCRIPT(ch)->script->prev->next = script;
		  else
			SHAPE_SCRIPT(ch)->root = script;
