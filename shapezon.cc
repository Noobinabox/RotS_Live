...
int command_room(struct reset_com * com){
  ...
  case 'D': case 'R':
    ...
    break;
  case 'P':
    if(com->arg1 >= 0) rom = com->arg1;
    else rom = shapezone_lastroom;
    break;
  case 'L':
    if(com->arg2 >= 0) rom = com->arg2;
    else rom = shapezone_lastroom;
    break;
  default: rom = 0;
    break;
...
void set_command_room(struct reset_com * com, int rom){
  ...
  switch(com->command){
  case 'M': case 'O': case 'L':
    com->arg2 = rom;
    break;
  case 'G': case 'E': case 'K': case 'A':
    shapezone_lastroom = rom;
    break;
  case 'D': case 'R': case 'P':
    com->arg1 = rom;
    break;
  default: 
    break;
...
//     break;
  ...
  case 'D':   /* set state of door */
    switch(com->arg2){
    case NORTH: tmpc='N'; break;
    case EAST: tmpc='E'; break;
    case SOUTH: tmpc='S'; break;
    case WEST: tmpc='W'; break;
    case UP: tmpc='U'; break;
    case DOWN: tmpc='D'; break;
    default: tmpc='X';
    }
...
com->if_flag,com->arg1,com->arg2,com->arg3,com->arg4,com->arg5);
  ...
  case 'M':  case 'N':  case 'H':  case 'E': case 'Q': case 'K':
  ...
  if(zon->comment) fprintf(f," %s",zon->comment);
...
void implement_zone(struct char_data * ch){
  ...
  if(!IS_SET(SHAPE_ZONE(ch)->flags,SHAPE_ZONE_LOADED)){
    ...
    SHAPE_ZONE(ch)->procedure=SHAPE_NONE;
  ...
  if((zone_table[adr].symbol != SHAPE_ZONE(ch)->symbol) ||
...
#define DESCRCHANGE(line,addr)do{\
  if(!IS_SET(SHAPE_ZONE(ch)->flags,SHAPE_SIMPLE_ACTIVE)){\
    ...
    str[0]=0;\
    SHAPE_ZONE(ch)->tmpstr = str_dup(addr);\
...
#define DIGITCHANGE(line,num)\
	...
	tmp2=0;	for(i=0;i<num;i++){\
	  while((arg[tmp2]<'0') && (arg[tmp2]!='-') && (arg[tmp2])) tmp2++;\
	  tmp[i]=atoi(arg+tmp2);\
	  while((arg[tmp2]>' ') && (arg[tmp2])) tmp2++;	}\
      }\
            shape_standup(ch,SHAPE_ZONE(ch)->position);\
          ch->specials.prompt_number=7;\
      REMOVE_BIT(SHAPE_ZONE(ch)->flags,SHAPE_DIGIT_ACTIVE);\
...
#define REALDIGCHANGE(line,addr)do{\
      if(!IS_SET(SHAPE_ZONE(ch)->flags,SHAPE_DIGIT_ACTIVE)){\
...
//  char key;
  ...
  if(SHAPE_ZONE(ch)->editflag==0){
    sscanf(arg,"%s",str);
    if( (str[0]>='0') && (str[0]<='9') ){
    SHAPE_ZONE(ch)->editflag=atoi(str);
    str[0]=0;
    if( SHAPE_ZONE(ch)->editflag==0){
      list_help_zone(ch);
      return;
  ...
  if(IS_SET(SHAPE_ZONE(ch)->flags,SHAPE_CURRFLAG))
    SHAPE_ZONE(ch)->cur_room = world[ch->in_room].number;
...
while(SHAPE_ZONE(ch)->editflag)
  ...
  case 1:                  /* show current  */
	...
	send_to_char(str,ch);
      ...
      else send_to_char("No previous command.\n\r",ch);
	...
	send_to_char(str,ch);
      ...
      else send_to_char("No next command.\n\r",ch);
    }
    else send_to_char("No current command.\n\r",ch);
    SHAPE_ZONE(ch)->editflag=0;
    break;
  ...
  case 2:    /*Set mask*/
	...
	return;
      ...
      SHAPE_ZONE(ch)->mask.arg7=-1;
                   ...
                   if(st1[0]) SHAPE_ZONE(ch)->mask.command=st1[0];
      if((tmp1>1)&&(st2[0]!='*')&&(st2[0])) SHAPE_ZONE(ch)->mask.if_flag=atoi(st2);
      if((tmp1>2)&&(st3[0]!='*')&&(st3[0])) SHAPE_ZONE(ch)->mask.arg1=atoi(st3);
      if((tmp1>3)&&(st4[0]!='*')&&(st4[0])) SHAPE_ZONE(ch)->mask.arg2=atoi(st4);
      if((tmp1>4)&&(st5[0]!='*')&&(st5[0])) SHAPE_ZONE(ch)->mask.arg3=atoi(st5);
      if((tmp1>5)&&(st6[0]!='*')&&(st6[0])) SHAPE_ZONE(ch)->mask.arg4=atoi(st6);
      if((tmp1>6)&&(st7[0]!='*')&&(st7[0])) SHAPE_ZONE(ch)->mask.arg5=atoi(st7);
      if((tmp1>7)&&(st8[0]!='*')&&(st8[0])) SHAPE_ZONE(ch)->mask.arg6=atoi(st8);
      if((tmp1>8)&&(st9[0]!='*')&&(st9[0])) SHAPE_ZONE(ch)->mask.arg7=atoi(st9);
    }
  ...
  case 3:    /*Set command*/
	...
	return;
      ...
      switch(st1[0]){
      case 'M': case 'N': case 'O': case 'G': case 'H': case 'E':
      case 'Q': case 'P': case 'D': case 'L': case 'A': case'K':
      case 'm': case 'n': case 'o': case 'g': case 'h': case 'e':
      case 'q': case 'p': case 'd': case 'l': case 'a': case 'k':
	SHAPE_ZONE(ch)->curr->comm.command=toupper(st1[0]);
	SHAPE_ZONE(ch)->editflag=4;
	break;
      default:
  ...
  case 4:
    ...
    case 'A':
      ...
      tmp[3]=tmp[2]; tmp[2]=tmp[1]; tmp[1]=tmp[0]; tmp[0]=1;
      break;
    case 'O': case 'G':
    ...
    case 'D':
      ...
      SHAPE_ZONE(ch)->editflag=5;
      break;
    case 'M':  case 'N':  case 'H':  case 'E': case 'Q':
    ...
    if(SHAPE_ZONE(ch)->editflag){
      SHAPE_ZONE(ch)->curr->comm.if_flag=tmp[0];
      SHAPE_ZONE(ch)->curr->comm.arg1=tmp[1];
      SHAPE_ZONE(ch)->curr->comm.arg2=tmp[2];
      SHAPE_ZONE(ch)->curr->comm.arg3=tmp[3];
      SHAPE_ZONE(ch)->curr->comm.arg4=tmp[4];
      SHAPE_ZONE(ch)->curr->comm.arg5=tmp[5];
      SHAPE_ZONE(ch)->curr->comm.arg6=tmp[6];
      SHAPE_ZONE(ch)->curr->comm.arg7=tmp[7];
      SHAPE_ZONE(ch)->editflag=5;
    }
  ...
  case 6:         /* Choose next */
	...
	if(!SHAPE_ZONE(ch)->cur_room || 
	   (SHAPE_ZONE(ch)->cur_room == tmpzon->room)) break;
	tmpzon = tmpzon->next;
      }
    if(tmpzon){
      SHAPE_ZONE(ch)->curr = tmpzon;
  ...
  case 9:        /* remove command */
    ...
    if(tmpzon){
      ...
      if(tmpzon->next) (tmpzon->next)->prev=tmpzon;
    }
    else{
	...
	break;;
      ...
      SHAPE_ZONE(ch)->root=SHAPE_ZONE(ch)->curr->next;
  ...
  case 10:         /* insert after */
    ...
    if(SHAPE_ZONE(ch)->curr->next) (SHAPE_ZONE(ch)->curr->next)->prev=tmpzon; 
    tmpzon->next=SHAPE_ZONE(ch)->curr->next;
    SHAPE_ZONE(ch)->curr->next=tmpzon;
    CREATE(tmpzon->comment, char, 1);
    tmpzon->comment[0]=0;
    SHAPE_ZONE(ch)->curr=SHAPE_ZONE(ch)->curr->next;
    renum_rooms(SHAPE_ZONE(ch)->root);
  ...
  case 11:        /* insert before */
    ...
    if(SHAPE_ZONE(ch)->curr->prev) (SHAPE_ZONE(ch)->curr->prev)->next=tmpzon;
    else {
      SHAPE_ZONE(ch)->root=tmpzon;
      tmpzon->prev=0;
    }
    SHAPE_ZONE(ch)->curr->prev=tmpzon;
    CREATE(tmpzon->comment, char, 1);
    tmpzon->comment[0]=0;
    SHAPE_ZONE(ch)->curr=SHAPE_ZONE(ch)->curr->prev;
    renum_rooms(SHAPE_ZONE(ch)->root);    
  ...
  case 13:
    if(!SHAPE_ZONE(ch)->curr->next){
      ...
      if(tmpzon->next) tmpzon->next->prev = SHAPE_ZONE(ch)->curr;
      SHAPE_ZONE(ch)->curr->next = tmpzon->next;

      if(SHAPE_ZONE(ch)->curr->prev)
	SHAPE_ZONE(ch)->curr->prev->next = tmpzon;
      else SHAPE_ZONE(ch)->root = tmpzon;
      tmpzon->prev = SHAPE_ZONE(ch)->curr->prev;
  ...
  case 14:            /* add owner */
    ...
    SHAPE_ZONE(ch)->root_owner=tmpowner;
  ...
  case 15:            /* remove owner */
    ...
    if(tmpowner==SHAPE_ZONE(ch)->root_owner) 
      SHAPE_ZONE(ch)->root_owner=tmpowner->next;
    RELEASE(tmpowner);
    SHAPE_ZONE(ch)->editflag=0;
  ...
  case 16:
    ...
    SHAPE_ZONE(ch)->x = tmp[0];
    SHAPE_ZONE(ch)->y = tmp[1];
    SHAPE_ZONE(ch)->editflag=0;
    break;
  case 17:
    if(ch->player.level<LEVEL_GRGOD){
    ...
    if(!IS_SET(SHAPE_ZONE(ch)->flags,SHAPE_DIGIT_ACTIVE)){
      ...
      if(arg[0] >= ' ') SHAPE_ZONE(ch)->symbol = arg[0];
      SHAPE_ZONE(ch)->editflag=0;
      shape_standup(ch,SHAPE_ZONE(ch)->position);
      ch->specials.prompt_number=7;
      REMOVE_BIT(SHAPE_ZONE(ch)->flags,SHAPE_DIGIT_ACTIVE);
  ...
  case 24:
    ...
    DIGITCHANGE("Enter RESET MODE (0-3):",1);
      SHAPE_ZONE(ch)->editflag=5;
    SHAPE_ZONE(ch)->reset_mode = tmp[0];
    SHAPE_ZONE(ch)->editflag=0;
    break;
  case 25:
  ...
  case 51:
    ...
    sprintf(str,"Owners: ");
    tmpowner=SHAPE_ZONE(ch)->root_owner;
    if(tmpowner->owner==0) strcat(str,"All");
    else
...
static char str[MAX_STRING_LENGTH];
  ...
  sprintf(str,"Mask is: %c ",mask->command);
    if(mask->if_flag!=-1) sprintf(str+strlen(str),"%d ",mask->if_flag);
  else sprintf(str+strlen(str),"* ");
 
 if(mask->arg1!=-1) sprintf(str+strlen(str),"%d ",mask->arg1);
    else sprintf(str+strlen(str),"* ");
  if(mask->arg2!=-1) sprintf(str+strlen(str),"%d ",mask->arg2);
  else sprintf(str+strlen(str),"* ");
  if(mask->arg3!=-1) sprintf(str+strlen(str),"%d ",mask->arg3);
  else sprintf(str+strlen(str),"* ");
  if(mask->arg4!=-1) sprintf(str+strlen(str),"%d ",mask->arg4);
  else sprintf(str+strlen(str),"* ");
  if(mask->arg5!=-1) sprintf(str+strlen(str),"%d ",mask->arg5);
  else sprintf(str+strlen(str),"* ");
  if(mask->arg6!=-1) sprintf(str+strlen(str),"%d ",mask->arg6);
  else sprintf(str+strlen(str),"* ");
  if(mask->arg7!=-1) sprintf(str+strlen(str),"%d\n\r",mask->arg7);
  else sprintf(str+strlen(str),"*\n\r");
  send_to_char(str,ch);
  str[0]=0;
...
int load_zone(struct char_data * ch,char * arg){
  ...
  if(tmp==-1) {send_to_char("no zone here.\n\r",ch); fclose(f);return -1;}
  ...
  SHAPE_ZONE(ch)->permission=0;
  owner=SHAPE_ZONE(ch)->root_owner;
  do{
    fscanf(f,"%d",&tmp);
    owner->owner=tmp;
    if( tmp==ch->specials2.idnum ) SHAPE_ZONE(ch)->permission=1;
    if(tmp){
  ...
  SHAPE_ZONE(ch)->zone_number=number;
  zon=SHAPE_ZONE(ch)->root;
  SHAPE_ZONE(ch)->curr=zon;
  prv=0;
  num=0;
  ...
  comm_num = 0;
   while(1){
    ...
    if(str[0]=='*'){
      ...
      str[strlen(str)-1]=0;
      if ((int)strlen(str) + 1 <= tmp) tmp = strlen(str);
...
*/
  ...
  if(!check) {sprintf(str,"no zone #%d in this file\n",num);
...
void free_zone(struct char_data * ch){
  ...
  struct owner_list * owner, * owner2;
	...
	tmp=tmp2;
      ...
      SHAPE_ZONE(ch)->root=0;
      SHAPE_ZONE(ch)->curr=0;
...
*/
  ...
  send_to_char("you invoked some rhymes from shapeless indefinity...\n\r",ch);
    ...
    do{
      ...
      if(!strncmp(str,"current",strlen(str))) {comm_key=SHAPE_CURRENT;break;}
...
/*    SHAPE_ZONE(ch)->procedure=comm_key;*/
  ...
  else comm_key=SHAPE_ZONE(ch)->procedure;
  switch(comm_key){
  case SHAPE_CREATE:
  ...
  case SHAPE_LOAD:
    ...
    else send_to_char("you already have something to care about\n\r",ch);
    SHAPE_ZONE(ch)->procedure=SHAPE_EDIT;
    break;
  case SHAPE_SAVE:
    if(SHAPE_ZONE(ch)->permission || GET_LEVEL(ch)>=LEVEL_GRGOD)
      replace_zone(ch,argument);
    else send_to_char("You are not allowed to save this zone.\n\r",ch);
    SHAPE_ZONE(ch)->procedure=SHAPE_EDIT;
    break;
  case SHAPE_IMPLEMENT:
    if(SHAPE_ZONE(ch)->permission || GET_LEVEL(ch)>=LEVEL_GRGOD)
      implement_zone(ch);
    else send_to_char("You are not allowed to implement this zone.\n\r",ch);
    SHAPE_ZONE(ch)->procedure=SHAPE_EDIT;
    break;
  case SHAPE_DONE:
    if(SHAPE_ZONE(ch)->permission || GET_LEVEL(ch)>=LEVEL_GRGOD){
