...
void recalc_skills(struct char_data *ch) {
  ...
  for(skill_no=1; skill_no<MAX_SKILLS; skill_no++){
	...
	skill_level * (1000 - GET_PROF_COOF
		       ((int) skills[skill_no].type, ch)) / 30; 
      
      if(skill_level < 20)
	tmps = tmps * (80 + skill_level)/100 + 200 - 
	  skills[(int) skill_no].level*10;
    ...
    if(tmps < 0) ch->knowledge[skill_no]=100;
  }
...
SPECIAL(guild) {
  ...
  if(!*arg) {
	...
	if((skills[tmp].level <= level) && 
	   (!IS_SET(skills[tmp].learn_type, LEARN_SPEC) || 
...
ACMD(do_pracreset)
{
  ...
  act("$n waves $s hand over your brow and you feel yourself a total rookie.\n\r"
      "Your practices have been reset",FALSE, ch, 0, vict, TO_VICT);
}
...
SPECIAL(kit_room)
{
  ...
  for (i = 1; i <= MAX_PROFS; ++i)
    if (GET_PROF_POINTS(i, ch) >= 
	GET_PROF_POINTS(coef, ch))
      coef = i;
  ...
  for (i = 0; i < kit_item_num; ++i) {
    if (level <= kit_eq[i].level &&
	IS_SET(1 << GET_RACE(ch), kit_eq[i].race) &&
  ...
  if (!num_items)
	...
	FALSE, ch, NULL, NULL, TO_CHAR);
  else if (level > KIT_MIDBIE &&
	   level <= KIT_LEGEND)
    act("You receive a few useful items.",
	FALSE, ch, NULL, NULL, TO_CHAR);
  else if (level > KIT_LEGEND &&
	   level <= KIT_ALWAYS)
...
SPECIAL(gatekeeper) {
  ...
  if (callflag != SPECIAL_NONE &&
      callflag != SPECIAL_SELF &&
      callflag != SPECIAL_COMMAND &&
...
void boot_ferries() {
  ...
  for(num = 0; num < num_of_ferries; num++)
    for(tmp = 0; tmp < ferry_boat_data[num].length; tmp++){
      ferry_boat_data[num].from_room[tmp] = 
	real_room(ferry_boat_data[num].from_room[tmp]);
      ferry_boat_data[num].to_room[tmp] = 
	real_room(ferry_boat_data[num].to_room[tmp]);
    }
  ...
  for(num = 0; num < num_of_captains; num++) {
    ...
    ferry_captain_data[num].num_of_ferry = 
      real_object(ferry_captain_data[num].num_of_ferry);
  }
}
...
void _recursive_move(struct char_data *ch, struct obj_data *hostobj){
  ...
  if((num < 0) || (num >= num_of_ferries))
    num = 0;	
  for(tmp = 0; tmp < ferry_boat_data[num].length; tmp++)
    if(ch->in_room == ferry_boat_data[num].from_room[tmp]) break;
  ...
  for (tmpfol = ch->followers; tmpfol; tmpfol = tmpfol->next)
	...
	act("ACK! $N could not follow you in.",FALSE, ch, 0, 
      ...
      else _recursive_move(tmpfol->follower,hostobj);
    }
}
...
SPECIAL(ferry_captain){
  ...
  for(ferryobj = world[tmp].contents; ferryobj;
    ...
    if(ferryobj->item_number == ferry_captain_data[num].num_of_ferry) break;
  }
  if(!ferryobj)
  ...
  if(!ferryobj) {
    if(!number(0,5)) do_say(host, "I wonder where my ship is?", 0, 0, 0);
    return FALSE;
  }
...
SPECIAL(mob_ranger) {
  ...
  if (host->in_room == NOWHERE)
    return 0;
  if ((callflag != SPECIAL_SELF) &&
      (callflag != SPECIAL_ENTER))
    return 0;
  
  if (GET_POS(host) < POSITION_FIGHTING) update_pos(host);
  tmpch = 0;
  if ((callflag == SPECIAL_ENTER) && ch && IS_AGGR_TO(host, ch))
    ...
    for (tmpch = world[host->in_room].people; tmpch; 
	 tmpch = tmpch->next_in_room)
      if ((tmpch != host) && CAN_SEE(host, tmpch) && 
	  IS_AGGR_TO(host, tmpch))
	break;
  }
  /* Ambush */
  if (tmpch) {
  ...
  mintime = 999; mintmp = NUM_OF_TRACKS;
...
char * dance_description [] [2] = {
  {"jumps merrily up and down, dancing happily."
   ,"You happily jump up and down"},
...
#define BLOCK_CHANCE(host, ch, width)                            \
  (((GET_SKILL(host, SKILL_BLOCK) + 110) / (1 + width) /         \
    (CAN_SEE(host, ch) ? 1 : 2) /                                \
    (GET_POS(host) < POSITION_STANDING ? 3 : 1) - 3 * width) *   \
...
SPECIAL(block_exit_north)
{
  ...
  if((callflag == SPECIAL_COMMAND) && (ch == host) && 
    ...
    if(cmd == CMD_BLOCK) return FALSE;
    if(!CAN_GO(ch, 0)) return FALSE;
  ...
  if((callflag != SPECIAL_COMMAND) || (cmd != 1)) return FALSE;
...
SPECIAL(block_exit_east){
  ...
  if((callflag != SPECIAL_COMMAND) || (cmd != 2)) return FALSE;
...
SPECIAL(block_exit_south){
  ...
  if((callflag != SPECIAL_COMMAND) || (cmd != 3)) return FALSE;
...
SPECIAL(block_exit_west){
  ...
  if((callflag != SPECIAL_COMMAND) || (cmd != 4)) return FALSE;
...
SPECIAL(block_exit_up){
  ...
  if((callflag != SPECIAL_COMMAND) || (cmd != 5)) return FALSE;
...
SPECIAL(block_exit_down){
  ...
  if((callflag != SPECIAL_COMMAND) || (cmd != 6)) return FALSE;
    ...
    if(GET_LEVEL(ch) == 6) {
      if(ch->specials2.rerolls >= 21) {
        ...
        wtltmp.targ2.ptr.text = 
          get_from_txt_block_pool("Sorry, you have exceeded the amount of times you can reroll statistics.");
        wtltmp.cmd = CMD_TELL;
        wtltmp.subcmd = 0;
...
SPECIAL(react_trap)
{
	...
	if (callflag == SPECIAL_COMMAND) {
		if (cmd_info[cmd].minimum_position > POSITION_SITTING &&
		    ch == host && cmd != CMD_TRAP)
...
SPECIAL(ar_tarthalon){                   // Ar-Tarthalon summons help when fighting
  ...
  if (number(0,1)){
	...
	switch (number(0, 6)){
	case 4:do_say(host, "Turn these intruders to dust, my faithful wizard!", 0,0,0);break;
	case 5:do_say(host, "Balkazor, burn these infidels into eternity!",0,0,0);break;
	}
  }
  else {
	...
	switch (number(0,6)) {
	case 4:do_say(host, "Karahaz, may these mere mortals feel the power of your blade!",0,0,0);break;
	case 5:do_say(host, "I require your service once again Karahaz - do not fail me.",0,0,0);break;
	}
  }
  if (!victim)
    return 0;
  if(GET_POS(victim) == POSITION_STANDING){
    ...
    case 8484: tmpwtl.cmd = 3;
      do_unlock(victim, "northdoor", 0, 0, 0);
      do_open(victim, "northdoor", 0, 0, 0);
      break;
    case 8485: tmpwtl.cmd = 1;
      do_unlock(victim, "southdoor", 0, 0, 0);
      do_open(victim, "southdoor", 0, 0, 0);
      break;
    case 8482: tmpwtl.cmd = 2;  break;
    case 8481: tmpwtl.cmd = 2;  break;
    case 8480: tmpwtl.cmd = 3;  break;
    case 8486: tmpwtl.cmd = 5;  break;
    case 8437: tmpwtl.cmd = 6;  break;
    case 15286: tmpwtl.cmd = 4;  break;
    case 15285: tmpwtl.cmd = 4;  break;
    case 15284: tmpwtl.cmd = 4;  break;
...
SPECIAL(ghoul){                         
  ...
  if (num_followers < 3){
    act("Decaying hands claw their way out of the ground as $n emerges."
	,TRUE, host, 0, 0, TO_ROOM);
...
SPECIAL(vampire_huntress){                  // Thuringwethil - in bat form she hunts through her zone
  ...
  if ((GET_POS(host) != POSITION_FIGHTING) && (!IS_SET(host->specials.affected_by, AFF_BASH))){  // day.
    ...
    case 15375: to_room = 15374; break;
    case 15374: to_room = 15373; break;
    case 15373: to_room = 15363; break;
    case 15363: to_room = 15362; break;
    case 15362: to_room = 15352; break;
    case 15352: to_room = 15351; break;
    case 15351: to_room = 15350; break;
    case 15350: to_room = 15342; break;
    case 15342: to_room = 15343; break;
    case 15343: to_room = 15333; break;
    case 15333: to_room = 15332; break;
    case 15332: to_room = 15331; break;
    case 15331: to_room = 15321; break;
    case 15321: to_room = 15311; break;
    case 15311: to_room = 15310; break;
    case 15310: to_room = 15334; break;
    case 15334: to_room = 15335; break;
    case 15335: to_room = 15336; break;
    case 15336: to_room = 15337; break;
    case 15337: to_room = 15344; break;
    case 15344: to_room = 15345; break;
    case 15345: to_room = 15346; break;
    case 15346: to_room = 15347; break;
    case 15347: to_room = 15349; break;
    case 15349: to_room = 15359; break;
    case 15359: to_room = 15369; break;
    case 15369: to_room = 15368; break;
    case 15368: to_room = 15367; break;
    case 15367: to_room = 15377; break;
    case 15377: to_room = 15376; break;
    case 15376: to_room = 15375; break;
    default: to_room = 15375; break;
    }
	...
	if (tmpno == 1){
	  if (IS_RIDING(victim)) stop_riding(victim);
	  ...
	  if (tmpno == 2 && GET_LEVEL(victim) < LEVEL_GOD)
...
SPECIAL(vortex_elevator){ // For now, this little func serves the sole and only purpose ...
  ...
  for (tmpchar = world[host->in_room].people; tmpchar; tmpchar=tmpchar->next_in_room) // for all players present in the room ...
    if (host!=tmpchar){ // For all mobs in room, except vortex ...
      ...
      if (!IS_NPC(tmpchar)) act("$n makes you very dizy!",FALSE, host, 0, tmpchar, TO_VICT); // and tell it tmpchar , unless ofcourse he's a mob
    }
  return 0;
}
...
SPECIAL(reciter){ // reciting mob
  ...
  if (GET_HIT(ch)<1) {quitting=1;goto remove_scroll;} // Hm, sbdy just has spoken his last line...
  if ((GET_POS(host)==POSITION_FIGHTING)) return 0;
  if (number(0,100)<50) return 0;
  if (c){ // singing already...
    e=strchr(c,'\n');
    if (e) {
      *e='\0';
      if (*c=='+') act((char*)(c+1),TRUE,host,0,0,TO_ROOM); else
	do_say(host,c,0,0,0);
      *e='\n';
      host->specials.recite_lines=e+2; // it appears, asif each line is terminated by a two character combination 
    } else host->specials.recite_lines=NULL;
    return 0;
  }
  // Not singing 
 remove_scroll:
  for (o=host->carrying;((o)&&(o->name)&&(strcmp(o->name,"textscroll")));o=o->next); // find a scroll
