 ...
 */
void show_mount_to_char(struct char_data* i, struct char_data* ch,
    ...
    for (; tmpch && char_exists(tmpnum); tmpch = tmpch->mount_data.next_rider) {
        if (CAN_SEE(ch, tmpch)) {
            ...
            if (ch == tmpch) {
                ...
                strcat(buf, !vis_count ? PERS(tmpch, ch, TRUE, FALSE) : PERS(tmpch, ch, FALSE, FALSE));
                if (color)
                    strcat(buf, CC_USE(ch, COLOR_CHAR));
            }
        ...
        case ROOMAFF_SPELL:
            *buf2 = 0;
            ...
            sprintf(str, "%s Spell %s(%d) level %d, %dhrs, sets %s.\r\n",
                str, ((aff->location >= 0) && (aff->location < MAX_SKILLS)) ? skills[aff->location].name : "none", aff->location,
                ...
                if (!tmp && (!IS_SET(EXIT(ch, door)->exit_info, EX_CLOSED)
                    ...
                    if (GET_LEVEL(ch) >= LEVEL_IMMORT) {
                        ...
                        if (!CAN_SEE(ch) && !PRF_FLAGGED(ch, PRF_HOLYLIGHT)) {
                            sprintf(buf + strlen(buf), "%-7s - Too dark to tell\n\r",
                                (((GET_RACE(ch) == RACE_URUK) || (GET_RACE(ch) == RACE_ORC)) && IS_SUNLIT_EXIT(tmp, ch->in_room, door)) ? sun_exits[door] : exits[door]);
                        } else {
                            sprintf(buf + strlen(buf), "%-7s - %s\n\r",
                                (((GET_RACE(ch) == RACE_URUK) || (GET_RACE(ch) == RACE_ORC)) && IS_SUNLIT_EXIT(tmp, ch->in_room, door)) ? sun_exits[door] : exits[door], world[ch->in_room].name);
                        }
                        ch->in_room = tmp;
                    }
                } else {
                    if (!IS_SET(EXIT(ch, door)->exit_info, EX_ISHIDDEN)) {
                        sprintf(buf + strlen(buf), "%-7s - Closed %s\n\r",
                            (((GET_RACE(ch) == RACE_URUK) || (GET_RACE(ch) == RACE_ORC)) && IS_SUNLIT_EXIT(ch->in_room, ch->in_room, door)) ? sun_exits[door] : exits[door], EXIT(ch, door)->keyword);
...
ACMD(do_toggle)
{
    ...
    sprintf(buf, "\r\nYou are employing %s tactics, and are speaking %s.\r\n",
        buf3, ch->player.language ? skills[ch->player.language].name : "common tongue");
    send_to_char(buf, ch);
...
void do_identify_object(struct char_data* ch, struct obj_data* j)
{
    ...
    sprintf(buf, "   You feel certain the object you have"
                 " is %s. \r\n",
        j->short_description ? j->short_description : "No object description found, please report. ");
    send_to_char(buf, ch);

    sprintf(buf, "%s \r\n", j->action_description ? j->action_description : "No object description, please report. \r\n");
    send_to_char(buf, ch);
