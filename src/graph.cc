...
#define IS_CLOSED(x, y) (IS_SET((x)->dir_option[(y)]->exit_info, EX_LOCKED | EX_ISHIDDEN) && \
			  (IS_SET((x)->dir_option[(y)]->exit_info, EX_CLOSED) || \
                           !IS_SET((x)->dir_option[(y)]->exit_info, EX_ISDOOR)))
#else
...
char * track_desc(int track_age) {
  switch (track_age) {
  case 0: return "fresh";
  case 1: return "very clear";
  case 2: return "clear";
  case 3:
  case 4: return "quite clear";
  case 5:
  case 6:
  case 7: return "distinctive";
  case 8:
  case 9:
  case 10: return "weathering";
  case 11:
  case 12:
  case 13:
  case 14:
  case 15: return "weathered";
  case 16:
  case 17:
  case 18:
  case 19:
  case 20: return "very poor";
  case 21:
  case 22:
  case 23:
  case 24: return "barely recognisable";
...
int show_tracks(char_data * ch, char * name, int mode){
  ...
  for (tmp = 0, count = 0; tmp < NUM_OF_TRACKS; tmp++) {
    ...
    shall_show = (ch_num != 0) && ((mode == 1) || (tr_time < 3)) &&
      (GET_SKILL(ch, SKILL_TRACK) + chance_factor - tr_time*2> number(0,99));
    if (shall_show && name && *name) {
      if (ch_num > 0) 
	shall_show = isname(name, mob_proto[ch_num].player.name, 1);
      else shall_show = !str_cmp(name, pc_race_keywords[-ch_num]);
    }
    if (shall_show) {
      count++;
	...
	sprintf(buf,"%sThe tracks of %s lead %s.  Their condition is %s\n\r", buf,
                (ch_num >= 0) ? mob_proto[ch_num].player.short_descr : 
		pc_star_types[-ch_num], dirs[ch_room->room_track[tmp].data & 7]
		, track_desc(tr_time));
    }
  }
  if (count != 0) {
