...
#define PRE_COMMAND tmp_mask=CALL_MASK(host);\
                     CALL_MASK(host) = 0;
#define POST_COMMAND CALL_MASK(host) = tmp_mask;


void
service_commands(struct char_data *host, char *arg, int cmd,
		 int callflag, struct waiting_type *wtl)
{
  ...
  case 'C':
	...
	(SPECIAL_LIST_AREA(host)->next[tmp] >= 0) && 
...
void
int_tostack(struct char_data *host, char *arg, int cmd,
	    int callflag, struct waiting_type *wtl)
{
  int value, tmp, tmp2;
  char_data * tmpch;
  ...
  case 'r':
    ...
    if(SPECIAL_LIST_TYPE(host)==TARGET_OBJ){
      ...
      if(tmpobj->carried_by) tmpch = tmpobj->carried_by;
    }
    if(tmpch)
      value = tmpch->in_room;
    else 
  ...
  case '=':
    ...
    if(SPECIAL_LIST_HEAD(host) < 0) 
      ...
      value = compare_list(host, SPECIAL_LIST_AREA(host)->field + 
			   (SPECIAL_LIST_HEAD(host)),
			   SPECIAL_LIST_AREA(host)->field + 
			   (SPECIAL_LIST_NEXT(host)));
    TO_STACK(host, value);
    REMOVE_LIST(host);
    REMOVE_LIST(host);
    break;
  ...
  case 'S':
    for(tmp=2, tmp2=SPECIAL_LIST_NEXT(host); ((tmp < 100) && 
        !(SPECIAL_LIST(host) == SPECIAL_LIST_AREA(host)->field[tmp2]));
...
void
int_tolist(struct char_data *host, struct char_data *ch, char *cmdline, 
	   char *arg, int cmd, int callflag, struct waiting_type *wtl)
{
  ...
  case 'f':
    if((SPECIAL_LIST_TYPE(host) != TARGET_CHAR) ||
       (!SPECIAL_LIST(host).ptr.ch->specials.fighting)) {
      REMOVE_LIST(host);
      TO_LIST(host,0,SPECIAL_NULL);
      TO_STACK(host, 0);
    }
...
void
service_get_old(struct char_data *host, struct char_data *ch, char *cmdline, 
		char *arg, int cmd, int callflag, struct waiting_type *wtl)
{
  switch(*arg) {
...
void
service_set_old(struct char_data *host, struct char_data *ch, char *cmdline, 
		char *arg, int cmd, int callflag, struct waiting_type *wtl)
{
  int tmpvar;
 ...
 */
void
question_proc(struct char_data *host)
{
  int tmp;
  char tmp_mask;
  char tmpstr[255];
 ...
 */
void 
int_fromstack(struct char_data *host, char *arg, int cmd,
		   int callflag, struct waiting_type *wtl)
{
  int val;
  ...
  case 'h':
    if((SPECIAL_LIST_TYPE(host) == TARGET_CHAR) &&
       (SPECIAL_LIST(host).ptr.ch))
      SPECIAL_LIST(host).ptr.ch->tmpabilities.hit = val;
    break;

  case 'm':
    if((SPECIAL_LIST_TYPE(host) == TARGET_CHAR) &&
       (SPECIAL_LIST(host).ptr.ch))
      SPECIAL_LIST(host).ptr.ch->tmpabilities.mana = val;
    break;
  case 'v':
    if((SPECIAL_LIST_TYPE(host) == TARGET_CHAR) &&
       (SPECIAL_LIST(host).ptr.ch))
      SPECIAL_LIST(host).ptr.ch->tmpabilities.move = val;
    break;  
  }
...
SPECIAL(intelligent)
{
  ...
  for(tmp = 0; tmp < SPECIAL_STACKLEN; tmp++)
    if((SPECIAL_LIST_AREA(host)->field[tmp].type == TARGET_CHAR) && 
       !char_exists(SPECIAL_LIST_REFS(host))) {
      SPECIAL_LIST_AREA(host)->field[tmp].type = SPECIAL_NULL;
      SPECIAL_LIST_AREA(host)->field[tmp].ptr.other = 0;
    }
  ...
  while((key != 0) && (key != ',') && (key != ';') && (cmd_count < 100)) {
	...
	SPECIAL_LIST_AREA(host)->field[tmp].ptr.other = 0;
	SPECIAL_LIST_AREA(host)->next[tmp] = -1;
	SPECIAL_LIST_AREA(host)->field[tmp].type = SPECIAL_VOID;
	SPECIAL_STACK(host)[tmp] = 0;
      }
      SPECIAL_LIST_HEAD(host) = 0;
      SPECIAL_LIST_AREA(host)->field[0].type = SPECIAL_MARK;
      SPECIAL_LIST_AREA(host)->next[0] = -1;
      SPECIAL_STACKPOINT(host) = 0;
...
//      printf("to_list command:'%c', type=%d\n",tmp,SPECIAL_LIST_TYPE(host));
    ...
    case 'L': 
	  ...
	  tmp = SPECIAL_LIST_AREA(host)->next[tmp])
	if((SPECIAL_LIST_TYPE(host) ==
	    SPECIAL_LIST_AREA(host)->field[tmp].type) &&
	   (SPECIAL_LIST(host).ptr.other ==
	    SPECIAL_LIST_AREA(host)->field[tmp].ptr.other) )
	  break;
      if(tmp) 
	TO_STACK(host,1);
      else
	...
	SPECIAL_STACK(host)[0] = -SPECIAL_STACK(host)[0];
      break;
	...
	SPECIAL_STACK(host)[0] = (0 < SPECIAL_STACK(host)[0]);
      break;
