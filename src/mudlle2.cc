...
void
TO_STACK(struct char_data *host, long x)
{
  int tmp;

  SPECIAL_STACK(host)[SPECIAL_STACKPOINT(host)] = x;

  if(SPECIAL_STACKPOINT(host) == SPECIAL_STACKLEN) {
    for(tmp = 0; tmp < SPECIAL_STACKLEN - 1; tmp++)
      SPECIAL_STACK(host)[tmp] = SPECIAL_STACK(host)[tmp + 1];
    SPECIAL_STACK(host)[SPECIAL_STACKPOINT(host)] = x;
  }
  else {
    SPECIAL_STACK(host)[SPECIAL_STACKPOINT(host)] = x;
...
long
STACK_VALUE(struct char_data *host)
{
  int tmp;
...
long
FROM_STACK(struct char_data *host)
{
  int tmp;
...
void
DOWN_STACK(struct char_data *host)
{
  int tmp;
  ...
  SPECIAL_STACK(host)[SPECIAL_STACKPOINT(host)-1] = tmp;
}



void
UP_STACK(struct char_data *host)
{
  int tmp;
  tmp = STACK_VALUE(host);
  memmove(SPECIAL_STACK(host)+sizeof(long), SPECIAL_STACK(host),
	  sizeof(long)*(SPECIAL_STACKLEN-1));
  SPECIAL_STACK(host)[0] = tmp;
}



void
TO_LIST(struct char_data *host, void *entr, int type) 
{
  ...
  SPECIAL_LIST_AREA(host)->next[(int) tmp] = SPECIAL_LIST_HEAD(host);
  SPECIAL_LIST_HEAD(host) = tmp;
  SPECIAL_LIST_TYPE(host) = type;
  SPECIAL_LIST(host).ptr.other = entr;
...
void
TO_LIST(struct char_data *host, target_data *newtarg)
{
  char tmp, tmp2, overfl;
  ...
  SPECIAL_LIST_AREA(host)->next[(int) tmp] = SPECIAL_LIST_HEAD(host);
  SPECIAL_LIST_HEAD(host) = tmp;
  SPECIAL_LIST_AREA(host)->field[(int) tmp] = *newtarg;
}
...
void
REMOVE_LIST(struct char_data *host)
{
  int tmp;
...
UP_LIST(struct char_data *host)
{
  ...
  for(tmp = SPECIAL_LIST_HEAD(host), tmp2 = 0; 
      (SPECIAL_LIST_AREA(host)->next[(int) tmp] >= 0) && 
      (tmp2 < SPECIAL_STACKLEN - 1);
...
void
DOWN_LIST(struct char_data *host)
{
  char tmp, tmp2, tmp3;
  ...
  for(tmp = SPECIAL_LIST_HEAD(host), tmp2 = 0,tmp3 = 0; 
      ...
      tmp = SPECIAL_LIST_AREA(host)->next[(int) tmp], tmp2++)
    tmp3 = tmp;
  SPECIAL_LIST_AREA(host)->next[(int) tmp3] = -1;
  SPECIAL_LIST_AREA(host)->next[(int) tmp] = SPECIAL_LIST_HEAD(host);
  SPECIAL_LIST_HEAD(host) = tmp;
...
int
CHECK_LIST(struct char_data *host, char val)
{
  if(SPECIAL_LIST_AREA(host)->field[(int) val].type == TARGET_CHAR)
    if(!char_exists(SPECIAL_LIST_AREA(host)->field[(int) val].ch_num)) {
      SPECIAL_LIST_AREA(host)->field[(int) val].type = TARGET_NONE;
      SPECIAL_LIST_AREA(host)->field[(int) val].ptr.other = 0;
      return 0;
    }
...
void
TEXT_LIST(struct char_data *host, char *targ)
{
...
int
compare_list(struct char_data *host, struct target_data *it1,
	     struct target_data *it2)
{
  ...
  case TARGET_OBJ: 
    tmp = strlen(it1->ptr.obj->name); if (tmp > 250) tmp = 250;
    strncpy(str1, it1->ptr.obj->name, tmp);
    str1[tmp] = 0;
    break;

  case TARGET_CHAR:
    tmp = strlen(it1->ptr.ch->player.name); if (tmp > 250) tmp = 250;
    strncpy(str1, it1->ptr.ch->player.name, tmp);
    str1[tmp] = 0;
    break;

  case TARGET_ROOM:
    tmp = strlen(it1->ptr.room->name); if (tmp > 250) tmp = 250;
  ...
  case TARGET_OBJ: 
    tmp = strlen(it2->ptr.obj->name); if (tmp > 250) tmp = 250;
    strncpy(str2, it2->ptr.obj->name, tmp);
    str2[tmp] = 0;
    break;

  case TARGET_CHAR:
    tmp = strlen(it2->ptr.ch->player.name); if (tmp > 250) tmp = 250;
    strncpy(str2, it2->ptr.ch->player.name, tmp);
    str2[tmp] = 0;
    break;

  case TARGET_ROOM:
    tmp = strlen(it2->ptr.room->name); if (tmp > 250) tmp = 250;
  ...
  case TARGET_OBJ: 
    if((it1->ptr.obj->item_number >= 0) && 
       (it1->ptr.obj->item_number == it2->ptr.obj->item_number))
      return 1;
    return 0;
    
  case TARGET_CHAR:
    if(IS_NPC(it1->ptr.ch) && IS_NPC(it2->ptr.ch) &&
       (it1->ptr.ch->nr == it1->ptr.ch->nr))
      return 1;
    return 0;
...
void
int_itemtostring(struct char_data *host)
{
  switch(SPECIAL_LIST_TYPE(host)) {
  case TARGET_NONE:
    return;
  ...
  default:
    SPECIAL_LIST_TYPE(host) = TARGET_TEXT;
    SPECIAL_LIST(host).ptr.text = get_from_txt_block_pool("Weird item");
    break;
  }
