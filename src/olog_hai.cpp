...
void apply_victim_delay(char_data *victim, int delay) {
  ...
  WAIT_STATE_FULL(victim, delay, CMD_BASH, 2, 80, 0, 0, 0,
                  AFF_WAITING | AFF_BASH, TARGET_IGNORE);
}
...
bool is_skill_valid(char_data *ch, const int &skill_id) {
  ...
  if (utils::is_shadow(*ch)) {
    send_to_char(
        "Hmm, perhaps you've spent to much time in the mortal lands.\r\n", ch);
    return false;
  }
...
void apply_stomp_affect(char_data *attacker, char_data *victim) {
  ...
  apply_victim_delay(victim, wait_delay);
  damage(attacker, victim, calculate_stomp_damage(*attacker, prob), SKILL_STOMP,
         0);
}
...
void apply_overrun_damage(char_data *attacker, char_data *victim) {
  ...
  apply_victim_delay(victim, wait_delay);
  damage(attacker, victim, calculate_overrun_damage(*attacker, prob),
         SKILL_OVERRUN, 0);
}
...
void apply_cleave_damage(char_data *attacker, char_data *victim) {
  ...
  damage(attacker, victim, calculate_cleave_damage(*attacker, prob),
         SKILL_CLEAVE, 0);
}
...
void generate_frenzy_message(char_data *character) {
  ...
  sprintf(
      buf,
      "$n enters a frenzied state, making $s strikes grow with fervor!\r\n");
...
void room_target(char_data *ch, void (*skill_damage)(char_data *character,
                                                     char_data *victim)) {
  char_data *victim = nullptr;
...
ACMD(do_overrun) {
  ...
  int total_moves =
      utils::get_prof_level(PROF_WARRIOR, *ch) / 8 + number(-1, 1);
  int loop_moves = 0;
  int dis;
...
ACMD(do_stomp) {
  ...
  olog_hai::do_sanctuary_check(ch);
  send_to_char(
      "You jump into the air and slam down feet first onto the ground!\n\r",
      ch);
