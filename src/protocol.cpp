...
static void ReportBug(char *apText) {
  // TODO: Replace with mudlog
  vmudlog(NRM, apText);
}
...
void MSDPSetTable(descriptor_t *apDescriptor, variable_t aMSDP,
  ...
  if (pProtocol != NULL && apValue != NULL) {
    if (*apValue == '\0') {
      ...
      char *pTable =
          (char *)malloc(strlen(apValue) + 3); /* 3: START, STOP, NUL */
...
void MSDPSendTable(descriptor_t *apDescriptor, variable_t aMSDP,
                   const char *apValue) {
  protocol_t *pProtocol = apDescriptor ? apDescriptor->pProtocol : NULL;
...
static void PerformHandshake(descriptor_t *apDescriptor, char aCmd,
  ...
  case (char)TELOPT_MXP:
    if (aCmd == (char)WILL || aCmd == (char)DO) {
      ...
      if (!pProtocol->bMXP) {
        /* Enable MXP. */
        char EnableMXP[] = {(char)IAC, (char)SB, TELOPT_MXP,
                            (char)IAC, (char)SE, '\0'};
  ...
  case (char)TELOPT_TTYPE:
    if (pProtocol->bTTYPE) {
      ...
      if (PrefixString("MTTS ", pClientName)) {
        pProtocol->pVariables[eMSDP_CLIENT_VERSION]->ValueInt =
            atoi(pClientName + 5);
    ...
    if (apDescriptor->pProtocol->Negotiated[aProtocol] != abWillDo) {
      ...
      if (abSendReply) {
        ...
        case eNEGOTIATED_TTYPE:
          SendNegotiationSequence(apDescriptor, abWillDo ? DO : DONT,
                                  TELOPT_TTYPE);
          break;
        case eNEGOTIATED_ECHO:
          SendNegotiationSequence(apDescriptor, abWillDo ? WILL : WONT,
                                  TELOPT_ECHO);
          break;
