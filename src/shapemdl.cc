...
int
load_mudlle(char_data *ch, char *arg)
{
  FILE *fp;
  ...
  if(num == 99999) {
    ...
    *tmpstr = 0;
    ...
    SHAPE_MUDLLE(ch)->real_num = real_program(number);
...
int
save_mudlle(struct char_data *ch)
{
  FILE *fp, *ofp;
  int tmp, num, saved;
  char str[MAX_STRING_LENGTH];
...
void
implement_mudlle(struct char_data *ch)
{
  if(!SHAPE_MUDLLE(ch)) {
    send_to_char("You are not shaping anything.\n\r",ch);
    return;
  ...
  mobile_program[SHAPE_MUDLLE(ch)->real_num] =
    mudlle_converter(SHAPE_MUDLLE(ch)->txt);
...
void
show_mudlle(struct char_data *ch)
{
  char str[500];
...
void
free_mudlle(struct char_data *ch)
{
...
void
edit_mudlle(struct char_data *ch)
{
  if(!ch->desc) 
    return;
...
void
shape_center_mudlle(struct char_data *ch, char *argument)
{
