...
void closing_time(struct char_data * keeper){
  ...
  for(dir = 0; dir < NUM_OF_DIRS; dir++){
	...
	do_open(keeper,EXIT(keeper,dir)->keyword,0,0,0);
      ...
      if(!IS_SET(EXIT(keeper,dir)->exit_info, EX_CLOSED) &&
	 (EXIT(keeper,dir)->to_room != NOWHERE)) break;
    }
  }
...
if ((IS_OBJ_STAT(temp1, ITEM_ANTI_EVIL) && IS_EVIL(keeper)) ||
       (IS_OBJ_STAT(temp1, ITEM_ANTI_GOOD) && IS_GOOD(keeper))){
...
SPECIAL(shop_keeper)
{
  ...
  /*
    ...
    */	    keeper = temp_char;
    ...
    if (((shop_index[shop_nr].open1 == time_info.hours) ||
	(shop_index[shop_nr].open2 == time_info.hours)) &&
    ...
    if ((((shop_index[shop_nr].close1 == time_info.hours) &&
	  ((shop_index[shop_nr].close1%24) != (shop_index[shop_nr].open2%24)))
	 || ((shop_index[shop_nr].close2 == time_info.hours) &&
	 ((shop_index[shop_nr].close2%24) != (shop_index[shop_nr].open1%24))))
	&&  (cmd <= 0) && (shop_index[shop_nr].is_open)){
      closing_time(keeper);
