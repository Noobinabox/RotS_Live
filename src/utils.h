...
#define RELEASE(pointer) do{\
        if((pointer) && global_release_flag) free_function(pointer);\
        (pointer) = 0; }while(0)

#define RECREATE(result,type, number, numb2) do{ void * tmpp = 0;\
                                  CREATE(tmpp, type, number);\
                if(numb2 > 0) memmove(tmpp, result, sizeof(type)*numb2);\
                                RELEASE(result); result = (type*)tmpp;}while(0)
/* #define CREATE(result, type, number) do{\
          if((number) == 0){\
...
#define NANA(num) (num == 8 ? "an" : num == 11 ? "an" : num == 18 ? "an" : \
		   (num >= 80 && num < 89) ? "an" : \
		   (num >= 800 && num < 900) ? "an" : \
		   (num >= 8000 && num < 9000) ? "an" : "a")
...
#define SET_PROF_LEVEL(prof,ch,val) do{if(prof!=PROF_GENERAL) (ch)->profs->prof_level[prof]=val; }while(0)
...
#define GET_PROF_COOF(prof,ch) ((prof == PROF_GENERAL) ? 1000 : \
	(GET_RACE((ch)) == RACE_ORC) ? (square_root[(ch)->profs->prof_coof[(prof)]] * 2 + 2) / 3 : \
...
#define IS_SUNLIT_EXIT(cur_room, adj_room, door) \
	((weather_info.sunlight == SUN_LIGHT) && \
	 (!IS_SET(world[adj_room].room_flags, DARK)) && \
	 (!IS_SET(world[adj_room].room_flags, SHADOWY)) && \
 	 (!IS_SET(world[adj_room].room_flags, INDOORS)) && \
	 ((!IS_SET(world[cur_room].dir_option[door]->exit_info, EX_CLOSED) || \
	  (IS_SET(world[cur_room].dir_option[door]->exit_info, EX_ISBROKEN)))))
...
#define GET_SKILL(ch, i) (IS_AFFECTED(ch, AFF_CONFUSE) ? \
	((((ch)->knowledge ? (GET_BODYTYPE((ch)) == 2) ? 0 : (((ch)->knowledge)[i]) : 80)) - get_confuse_modifier((ch))) : \
	((ch)->knowledge ? (GET_BODYTYPE(ch) == 2) ? 0 : (((ch)->knowledge)[i]) : 80))
...
#define GET_KNOWLEDGE(ch, i) (IS_AFFECTED(ch, AFF_CONFUSE) ? \
        ((((ch)->knowledge ? (((ch)->knowledge)[i]) : 80)) - get_confuse_modifier((ch))) : \
        ((ch)->knowledge ? (((ch)->knowledge)[i]) : 80))

#define SET_KNOWLEDGE(ch, i, pct) { if ((ch)->knowledge) (ch)->knowledge[i] = pct; }
...
#define WAIT_STATE_BRIEF(ch,cycle, commd, subcommd, prir, new_flag) do{\
           char_data * tmpch;\
	   ...
	   ch->delay.priority = prir;\
           ...
           tmpch = waiting_list;\
           if(tmpch == ch)  tmpch = waiting_list = ch->delay.next;\
           if(!tmpch)  waiting_list = ch;\
           else {\
             while(tmpch->delay.next){\
               if(tmpch->delay.next == ch) tmpch->delay.next=ch->delay.next;\
               else tmpch = tmpch->delay.next;\
             }\
             if(tmpch != ch) tmpch->delay.next = ch;\
           }\
	   ch->delay.next = 0;\
        }while(0)
...
#define IS_SHADOW(ch) ((IS_NPC(ch) && IS_SET(MOB_FLAGS(ch), MOB_SHADOW)) ||\
      (!IS_NPC(ch) && IS_SET(PLR_FLAGS(ch), PLR_ISSHADOW)))
...
#define IS_AGGR_TO(ch, vict) \
  (IS_NPC(ch) && \
   (((ch)->specials2.pref & (1 << GET_RACE(vict))) || \
   ((other_side(ch, vict) && !IS_NPC(vict)) \
    && GET_RACE(ch) != 0)))

#define WILL_TEACH(ch, vict) \
  (IS_NPC(ch) && \
  (((ch)->specials2.will_teach & (1 << GET_RACE(vict))) || \
  ((other_side(ch, vict) && !IS_NPC(vict)) \
  && GET_RACE(ch) != 0)))


#define RP_RACE_CHECK(ch, vict) (IS_NPC(ch) && (!(ch)->specials2.rp_flag)  || \
  ( (ch)->specials2.rp_flag & (1 << GET_RACE(vict)))    )
...
#define SET_SPEC(ch, value) do{if(!IS_NPC(ch)&&(ch)->profs)(ch)->profs->specialization = (value);}while(0)
