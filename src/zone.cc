...
renum_zone_one(int zone)
{
  ...
  for (comm = 0; comm < zone_table[zone].cmdno; comm++) {
    ...
    case 'A':
      ...
      case 5:
      case 6:
	a = zone_table[zone].cmd[comm].arg3 =
	  real_mobile(zone_table[zone].cmd[comm].arg3);
	break;
      }
      break;
    case 'L':
      zone_table[zone].cmd[comm].arg2 = 
	real_room(zone_table[zone].cmd[comm].arg2);
      switch (zone_table[zone].cmd[comm].arg1) {
      case 0:
      case 5:
      case 6:
	...
	real_mobile(zone_table[zone].cmd[comm].arg1);
      b = zone_table[zone].cmd[comm].arg2 = 
	real_room(zone_table[zone].cmd[comm].arg2);
      break;
    case 'O':
      a = zone_table[zone].cmd[comm].arg1 = 
	...
	real_room(zone_table[zone].cmd[comm].arg1);
      a = zone_table[zone].cmd[comm].arg2 = 
	...
	real_object(zone_table[zone].cmd[comm].arg2);
      zone_table[zone].cmd[comm].arg3 = 
	real_object(zone_table[zone].cmd[comm].arg3);
      zone_table[zone].cmd[comm].arg4 = 
	real_object(zone_table[zone].cmd[comm].arg4);
      zone_table[zone].cmd[comm].arg5 = 
	real_object(zone_table[zone].cmd[comm].arg5);
      zone_table[zone].cmd[comm].arg6 = 
	real_object(zone_table[zone].cmd[comm].arg6);
      zone_table[zone].cmd[comm].arg7 = 
	real_object(zone_table[zone].cmd[comm].arg7);
      a = b = 1;
      break;
    case 'D':
      a = zone_table[zone].cmd[comm].arg1 = 
 ...
 */
int
check_if_flag(int if_flag, int last_cmd, int last_mob, int last_obj)
{
	...
	if (invert_requirements) {
		...
		if (require_good_fame_lead &&
		    pkill_get_good_fame() > pkill_get_evil_fame())
			return 0;

		if (require_evil_fame_lead &&
		    pkill_get_evil_fame() > pkill_get_good_fame())
			return 0;
	} else {
...
#define ZCMD zone_table[zone].cmd[cmd_no]
  ...
  for (cmd_no = 0; cmd_no < zone_table[zone].cmdno; cmd_no++) {
	...
	case 3: /* last_obj from the inventory of last_mob */
	  ...
	  break;	  	  	  
	case 4:
	  if (last_mob && mob && ZCMD.arg4 >= 0 &&
	      ZCMD.arg4 < MAX_WEAR) {
	    tmpobj = mob->equipment[ZCMD.arg4];
	    if (tmpobj && ZCMD.arg3 >= 0 &&
		tmpobj->item_number != ZCMD.arg3)
	      tmpobj = 0;
	    if (tmpobj) {
	      obj = tmpobj;
	      last_obj = last_cmd = 1;
	  ...
	  if (ZCMD.arg2 >= 0 || ZCMD.arg3 >= 0) {
	    ...
	    for (tmp = 0, tmpmob = character_list; 
		 tmpmob; tmpmob = tmpmob->next) {
		  ...
		  tmpmob->nr == ZCMD.arg3 &&
		  tmpmob->in_room >= 0 && 
		  world[tmpmob->in_room].zone == tmp2)
		tmp++;
	      if (tmp >= ZCMD.arg4)
		break;
	    }
	...
	break;
      case 'A':
	if ((ZCMD.arg1 != 5 && ZCMD.arg1 != 10 && !mob) || 
	    (ZCMD.arg1 == 5 && !obj)) {
	  last_cmd = 0;
	  break;
	}
	...
	if ((!ZCMD.arg3 || obj_index[ZCMD.arg1].number < ZCMD.arg3) &&
	  ...
	  if (ZCMD.arg2 >= 0) {
	    if (!ZCMD.arg5 ||
		(count_obj_in_list(ZCMD.arg1,
				   world[ZCMD.arg2].contents) < ZCMD.arg5)) {
 ...
 */
void
put_to_reset_q_pool(struct reset_q_element *oldres)
{
