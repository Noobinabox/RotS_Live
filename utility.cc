...
int
get_race_weight(struct char_data *ch)
{
  int gender_mod;
...
int
get_race_height(struct char_data *ch)
{
  int gender_mod;
...
{
  return GET_PROF_LEVEL(PROF_CLERIC, ch) + GET_WILL(ch) -
    (get_confuse_modifier(ch) / 10);
}



int
get_exit_width(struct room_data *room, int dir)
{
...
int
string_to_new_value(char *arg, int *value)
{  
  while(*arg && (*arg <= ' ')) 
    arg++;
...
int
weight_coof(struct obj_data *obj)
{
...
int
armor_absorb(struct obj_data *obj)
{
  ...
  encumb_points = obj->obj_flags.value[2] * 6 + 
    GET_OBJ_WEIGHT(obj) / weight_coof(obj) / 20;
 ...
 */
int
get_real_stealth(struct char_data *ch)
{
  int percent;
...
void free_function(void * pnt){
  ...
  if(create_pointer) free(create_pointer);  
...
void encrypt_line(unsigned char * line, int len){
  ...
  for(tmp=0; tmp<len; tmp++) line[tmp] = lp[tmp];
}
...
void decrypt_line(unsigned char * lp, int len){
  ...
  for(tmp=0; tmp<len; tmp++) lp[tmp] = line[tmp];
}
...
char * strcpy_lang(char * str1, char * str2, byte freq, int maxlen){
  ...
  for(i=0; i<len; i++){
    if((number(1,100) > freq) && isalpha(str2[i])){
      if(isupper(str2[i])) str1[i] = number(65,90);
      else str1[i] = number(97,122);
    }
    else str1[i] = str2[i];
  }
  str1[i] = 0;
...
void reshuffle(int * arr, int len){
  ...
  for(tmp = 0; tmp < len; tmp++) flags[tmp] = 1;

  for(tmp = 0; tmp < len; tmp++){
    num = 1+number(0, len-tmp-1);
    for(tmp2 = 0; (tmp2 < len) && num; tmp2++) num -= flags[tmp2];
    if(tmp2 >= len+1){
      tmp2 = len-1;
  ...
  for(tmp = 0; tmp < len; tmp++) arr[tmp] = newarr[tmp];
  return;
}
...
CAN_SEE(struct char_data *sub)
{
  ...
  if(!IS_LIGHT((sub)->in_room) &&
     (!IS_AFFECTED((sub), AFF_INFRARED) && !PRF_FLAGGED((sub), PRF_HOLYLIGHT)
      && !(OUTSIDE(sub) && IS_AFFECTED((sub), AFF_MOONVISION) &&
 ...
 */
int
CAN_SEE(struct char_data *sub, struct char_data *obj, int light_mode)
{  
  int tmp;
   ...
   */
  if(GET_LEVEL(sub) < LEVEL_IMMORT && sub->in_room != obj->in_room &&
     IS_NPC(obj) && strcmp(GET_NAME(obj), "Guardian angel"))
    return 0;
  
  if(IS_SHADOW(obj) || IS_SHADOW(sub)) {
    if((sub->specials.fighting == obj) ||
       (obj->specials.fighting == sub))
      return 1;
    if(!(!IS_NPC(sub) && PRF_FLAGGED((sub), PRF_HOLYLIGHT)) &&
       (GET_PERCEPTION(sub) * GET_PERCEPTION(obj) < number(1, 10000))) 
      return 0;
  }
  ...
  if(!(IS_SHADOW(sub))) {
    if(GET_HIDING(obj)) {
      ...
      if(IS_NPC(sub) && IS_SET(obj->specials2.hide_flags, HIDING_SNUCK_IN) &&
	 !number(0, 9) && !IS_NPC(obj)) {
	act("$n glances directly at you, but doesn't seem to notice "
	    "your presence.", FALSE, sub, 0, obj, TO_VICT);
	return 0;
      }
      else
	tmp = see_hiding(sub);
      if((tmp < GET_HIDING(obj)) && 
    ...
    if(!light_mode) {
      if(!IS_LIGHT((obj)->in_room) && !IS_AFFECTED((sub), AFF_INFRARED) &&
	 !(!IS_NPC(sub) && PRF_FLAGGED((sub), PRF_HOLYLIGHT)) &&
	 !(OUTSIDE(sub) && IS_AFFECTED((sub), AFF_MOONVISION) &&
	   weather_info.moonlight))
	return 0;
 ...
 */
int
can_sense(char_data *sub, char_data *obj)
{
  if(!IS_AFFECTED((sub), AFF_DETECT_HIDDEN) || GET_PERCEPTION(obj) <= 0)
    return 0;

  if(30 + (GET_PROF_LEVEL(PROF_CLERIC, sub) * 3) >
     (100 - GET_PERCEPTION(obj) * GET_PERCEPTION(sub) / 100 + 
      GET_HIDING(obj) / 4))
    return 1;
  else
    return 0; 
...
int
CAN_SEE_OBJ(char_data *sub, obj_data *obj)
{
  if(!sub || !obj)
    return 0;
  ...
  if(IS_SHADOW(sub))
    if(IS_SET((obj)->obj_flags.extra_flags, ITEM_MAGIC) ||
       IS_SET((obj)->obj_flags.extra_flags, ITEM_WILLPOWER))
      return 1;
    else
      return 0;
  
  if((!IS_SET((obj)->obj_flags.extra_flags, ITEM_INVISIBLE) ||
      IS_AFFECTED((sub),AFF_DETECT_INVISIBLE)) && CAN_SEE(sub))
    return 1;
...
/* moved from utils.h */
int
CAN_GO(struct  char_data *ch, int door)
{
  if((EXIT(ch,door) && (EXIT(ch,door)->to_room != NOWHERE)) &&
     (!(IS_SHADOW(ch) ? (IS_SET(EXIT(ch, door)->exit_info, EX_DOORISHEAVY) &&
...
int
get_confuse_modifier(struct char_data *ch) 
{
...
int
get_power_of_arda(struct char_data *ch)
{
  struct affected_type *aff;
  int sun_mod;
...
int
has_critical_stat_damage(struct char_data *ch)
{
  if(((GET_STR(ch) * 100) / GET_STR_BASE(ch) < 30) ||
     ((GET_DEX(ch) * 100) / GET_DEX_BASE(ch) < 30) ||
     ((GET_CON(ch) * 100) / GET_CON_BASE(ch) < 30))
    return 1;
  else
    return 0;
}
...
int
can_breathe(struct char_data *ch)
{
  int result;
  ...
  if(world[ch->in_room].sector_type == SECT_UNDERWATER) {
    result = 0;
    if(IS_AFFECTED(ch, AFF_BREATHE) || IS_SHADOW(ch) ||
...
void
day_to_str(struct time_info_data *loc_time_info, char *str)
{
  char *s;
  int day;
...
int
find_player_in_table(char *name, int idnum)
{
  int i;

  for(i = 0; i <= top_of_p_table; i++)
    if(((idnum < 0) && (!str_cmp((player_table + i)->name, name))) ||
       ((player_table + i)->idnum == idnum))
      break;

  if((i > top_of_p_table) || 
     (IS_SET((player_table + i)->flags, PLR_DELETED)))
    return -1;
...
find_playing_char(int idnum)
{
  ...
  for(d = descriptor_list; d; d = d->next)
    if((d->character && d->connected == CON_PLYNG) &&
       (d->character->specials2.idnum == idnum))
      return d->character;
...
void
set_mental_delay(struct char_data *ch, int value)
{  
 ...
 */
int
compare_obj_to_proto(struct obj_data *obj)
{
  int diff = 0, i;
  struct obj_data *tmp;
  ...
  if (!tmp)								diff--;
    ...
    if (str_cmp(obj->name, tmp->name))					diff++;
    if (str_cmp(obj->description, tmp->description))			diff++;
    if (str_cmp(obj->short_description, tmp->short_description))	diff++;
    if (str_cmp(obj->action_description, tmp->action_description))	diff++;

    // Compare extra descriptions.
    edesc = obj->ex_description;
    tmp_edesc = tmp->ex_description;

    if (edesc && tmp_edesc) do {
      if (!((edesc->keyword == tmp_edesc->keyword) &&
	  (!str_cmp(edesc->description, tmp_edesc->description))))	diff++;
      edesc = edesc->next;
    ...
    if ((edesc && !tmp_edesc) ||
        (tmp_edesc && !edesc))						diff++;

    // Compare flags.
    if (obj->obj_flags.value[0] != tmp->obj_flags.value[0])		diff++;
    // See below for values 1, 2 and 3.
    if (obj->obj_flags.value[4] != tmp->obj_flags.value[4])             diff++;
    if (obj->obj_flags.type_flag != tmp->obj_flags.type_flag)		diff++;
    if (obj->obj_flags.wear_flags != tmp->obj_flags.wear_flags)		diff++;
    if (obj->obj_flags.extra_flags != tmp->obj_flags.extra_flags)	diff++;
    if (obj->obj_flags.cost != tmp->obj_flags.cost)			diff++;
    if (obj->obj_flags.cost_per_day != tmp->obj_flags.cost_per_day)	diff++;
    if (obj->obj_flags.timer != tmp->obj_flags.timer)			diff++;
    if (obj->obj_flags.bitvector != tmp->obj_flags.bitvector)		diff++;
    if (obj->obj_flags.level != tmp->obj_flags.level)			diff++;
    if (obj->obj_flags.rarity != tmp->obj_flags.rarity)			diff++;
    if (obj->obj_flags.material != tmp->obj_flags.material)		diff++;
    if (obj->obj_flags.prog_number != tmp->obj_flags.prog_number)	diff++;

    // Weight is special as this might be a container.
    if ((obj->obj_flags.type_flag != ITEM_CONTAINER) &&
	(obj->obj_flags.type_flag != ITEM_DRINKCON) &&
	(obj->obj_flags.weight != tmp->obj_flags.weight))		diff++;

    // Value 1 is special for drink containers.
    if ((obj->obj_flags.type_flag != ITEM_DRINKCON) &&
	(obj->obj_flags.value[1] != tmp->obj_flags.value[1]))		diff++;
    // Values 2 and 3 are special for light sources.
    if (obj->obj_flags.type_flag != ITEM_LIGHT) {
      if (obj->obj_flags.value[2] != tmp->obj_flags.value[2])		diff++;
      if (obj->obj_flags.value[3] != tmp->obj_flags.value[3])		diff++;
    }

    // Compare affections.
    for (i = 0; i < MAX_OBJ_AFFECT; i++) {
      if (!((obj->affected[i].location == tmp->affected[i].location) &&
          (obj->affected[i].modifier == tmp->affected[i].modifier)))	diff++;
    }
    ...
    if ((obj->obj_flags.type_flag == ITEM_WEAPON) &&
        (tmp->obj_flags.type_flag == ITEM_WEAPON)) {
      // Any type of enchant.
      if ((obj->affected[0].location == APPLY_OB) &&
          (tmp->affected[0].location != APPLY_OB)) 			diff--;
      ...
      if (IS_OBJ_STAT(obj, ITEM_MAGIC))	obj_flags += ITEM_MAGIC;
      if (IS_OBJ_STAT(obj, ITEM_ANTI_GOOD))	obj_flags += ITEM_ANTI_GOOD;
      if (IS_OBJ_STAT(obj, ITEM_ANTI_EVIL))	obj_flags += ITEM_ANTI_EVIL;

      if (IS_OBJ_STAT(tmp, ITEM_MAGIC))	tmp_flags += ITEM_MAGIC;
      if (IS_OBJ_STAT(tmp, ITEM_ANTI_GOOD))	tmp_flags += ITEM_ANTI_GOOD;
      if (IS_OBJ_STAT(tmp, ITEM_ANTI_EVIL))	tmp_flags += ITEM_ANTI_EVIL;

      // Now check the enchant.
      if ((obj->obj_flags.extra_flags != tmp->obj_flags.extra_flags) &&
	  (obj->obj_flags.extra_flags - obj_flags + tmp_flags
 				     == tmp->obj_flags.extra_flags))	diff--;
    }
  }
...
struct obj_data *obj_to_proto(struct obj_data *obj)
{
  ...
  for (i = 0; i < MAX_OBJ_AFFECT; i++) {
    ...
    if (tmp->affected[i].location > 0) aff_count = 1;
  }
  ...
  if (!aff_count) {
    ...
    if (obj->affected[0].location == APPLY_OB) {
      ...
      if (IS_OBJ_STAT(new_obj, ITEM_MAGIC))     obj_flags -= ITEM_MAGIC;
      if (IS_OBJ_STAT(new_obj, ITEM_ANTI_GOOD)) obj_flags -= ITEM_ANTI_GOOD;
      if (IS_OBJ_STAT(new_obj, ITEM_ANTI_EVIL)) obj_flags -= ITEM_ANTI_EVIL;
...
void
check_inventory_proto(struct char_data *ch)
{
  int result = 0;
  struct obj_data *tmp;
 ...
 */
char *
PERS(struct char_data *target, struct char_data *observer,
  ...
  if (CAN_SEE(observer, target) || force_visible) {
    if (other_side(observer, target)) {
      snprintf(name, 127, "%s%s%s",
	       ...
	       MOB_FLAGGED(target, MOB_ORC_FRIEND) &&
	       MOB_FLAGGED(target, MOB_PET) &&
	       other_side(target, observer)) {
      snprintf(name, 127, "%s%s%s",
	       CC_USE(observer, COLOR_ENMY),
	       pc_star_types[GET_RACE(target)],
	       CC_NORM(observer));
